...
I<[x]> := ideal< P | [
x[1]*x[5]^6 + 1/2*(-2*nu^3 - nu^2 + 2*nu)*x[1]^3*x[2]*x[5]^2 + (-2*nu^3 - 2*nu)*x[1]^3*x[3]*x[5]^2 + (-nu^3 - 2*nu^2 + nu)*x[3]*x[4]*x[5]^4 + 1/2*(2*nu^3 - 5*nu^2 - 2*nu)*x[1]^3*x[5]^2 + (nu^3 + nu)*x[4]*x[5]^4 + 1/2*(2*nu^3 + 2*nu + 1)*x[1]*x[2]*x[5]^2 + 1/2*(-3*nu^3 + 3*nu)*x[1]*x[3]*x[5]^2 + 1/2*(nu^3 + 4*nu^2 - nu)*x[1]^2*x[4] + 1/2*(nu^3 + nu - 9)*x[1]*x[5]^2,
x[1]*x[3]*x[5]^4 + 1/9*(4*nu^3 + 2*nu^2 - 4*nu)*x[1]*x[5]^4 + 1/18*(-10*nu^3 - 10*nu - 5)*x[1]^3*x[2] + 1/9*(-4*nu^3 - 11*nu^2 + 4*nu)*x[1]^3*x[3] + 1/9*(nu^3 + nu + 14)*x[3]*x[4]*x[5]^2 + 1/18*(-2*nu^3 - 2*nu + 17)*x[1]^3 + 1/9*(11*nu^3 + 10*nu^2 - 11*nu)*x[4]*x[5]^2 + 1/18*(-10*nu^3 - 5*nu^2 + 10*nu)*x[1]*x[2] + 1/3*(nu^3 + nu - 1)*x[1]*x[3] + 1/18*(2*nu^3 - 17*nu^2 - 2*nu)*x[1],
x[1]*x[4]*x[5]^4 + (-nu^3 - nu)*x[1]^2*x[2]*x[5]^2 + 1/2*(nu^3 - 2*nu^2 - nu)*x[1]^2*x[3]*x[5]^2 + 1/2*(nu^3 + nu - 2)*x[1]^2*x[5]^2 + (-nu^3 + nu)*x[2]*x[5]^2 + (-nu^3 - nu)*x[3]*x[5]^2 + 1/2*(nu^3 + nu + 2)*x[1]*x[4] + 2*nu^2*x[5]^2,
x[1]*x[2]^2*x[5]^2 + nu^2*x[2]*x[3]*x[4] - x[1]*x[5]^2 + (nu^3 + nu - 1)*x[2]*x[4] + (nu^3 + nu^2 - nu)*x[3]*x[4] - x[4],
x[1]^2*x[4]*x[5]^2 + 1/9*(nu^3 + 5*nu^2 - nu)*x[1]*x[5]^4 + 1/18*(-7*nu^3 - 7*nu + 1)*x[1]^3*x[2] + 1/18*(-11*nu^3 - 10*nu^2 + 11*nu)*x[1]^3*x[3] + 1/9*(7*nu^3 + 7*nu + 8)*x[3]*x[4]*x[5]^2 + 1/18*(4*nu^3 + 4*nu + 11)*x[1]^3 + 1/9*(5*nu^3 - 2*nu^2 - 5*nu)*x[4]*x[5]^2 + 1/18*(-7*nu^3 + nu^2 + 7*nu)*x[1]*x[2] + 1/6*(-nu^3 - nu - 2)*x[1]*x[3] + 1/18*(-4*nu^3 - 11*nu^2 + 4*nu)*x[1],
x[2]*x[5]^4 + (nu^3 - nu)*x[3]*x[5]^4 + 1/3*(2*nu^3 - 4*nu^2 - 2*nu)*x[1]*x[4]*x[5]^2 + x[5]^4 + 1/6*(-nu^3 + 2*nu^2 + nu)*x[1]^2*x[2] + 1/3*(nu^3 + nu - 2)*x[1]^2*x[3] + 1/6*(nu^3 - 2*nu^2 - nu)*x[1]^2 + 1/6*(nu^3 + nu - 2)*x[2] + 1/3*(nu^3 + nu^2 - nu)*x[3] + 1/6*(nu^3 + nu - 2),
x[1]^2*x[2]^2 - x[1]^2 + nu^2*x[2]^2 + nu^2,
x[1]^2*x[2]*x[3] + 1/3*(2*nu^3 + 2*nu^2 - 2*nu)*x[1]*x[4]*x[5]^2 + 1/3*(-2*nu^3 + nu^2 + 2*nu)*x[1]^2*x[2] + 1/3*(nu^3 + nu + 1)*x[1]^2*x[3] + 1/3*(-nu^3 - nu^2 + nu)*x[1]^2 + nu^2*x[2]*x[3] + 1/3*(2*nu^3 + 2*nu - 1)*x[2] + 1/3*(nu^3 + nu^2 - nu)*x[3] + 1/3*(-nu^3 - nu - 1),
x[1]^2*x[2]*x[4] + (-nu^3 + nu)*x[1]*x[2]*x[5]^2 + (-nu^3 - nu - 2)*x[1]*x[3]*x[5]^2 + x[1]^2*x[4] + 2*nu^2*x[1]*x[5]^2 + nu^2*x[2]*x[4] + (nu^3 + nu)*x[3]*x[4] + nu^2*x[4],
x[2]*x[3]*x[5]^2 - x[1]*x[2]*x[4] + (nu^3 - nu^2 - nu)*x[2]*x[5]^2 + (nu^3 + nu + 1)*x[3]*x[5]^2 - x[1]*x[4] - nu^2*x[5]^2,
x[2]*x[4]*x[5]^2 + (nu^3 - nu)*x[3]*x[4]*x[5]^2 + x[4]*x[5]^2 - x[1]*x[2] - nu^2*x[1]*x[3],
x[1]*x[3]*x[4] - x[2]*x[5]^2 + (-nu^3 + nu)*x[3]*x[5]^2 + nu^2*x[1]*x[4] - x[5]^2,
x[3]^2 - x[2],
x[4]^2 - x[3]
] >;
...
s`SolvableDBName := "128S151-8,32,2-g23-path1";
s`SolvableDBFilename := "128S151-8,32,2-g23-path1.m";
...
{ IntegerRing() | 124, 127 }
@};
s`SolvableDBBelyiMapTiming := 8.480p15;
s`SolvableDBLocalSanityCheckTiming := 3.090p15;
...
\[ 2, 8, 9, 7, 10, 1, 11, 18, 24, 22, 25, 26, 27, 28, 20, 19, 23, 5, 29, 3, 4, 6, 13, 38, 16, 33, 37, 15, 21, 43, 42, 44, 45, 41, 46, 40, 39, 14, 34, 12, 17, 35, 53, 36, 32, 51, 59, 60, 61, 56, 55, 58, 62, 57, 54, 30, 31, 48, 66, 70, 50, 49, 75, 74, 76, 77, 73, 78, 72, 71, 67, 47, 52, 68, 85, 69, 65, 83, 91, 92, 93, 88, 87, 90, 94, 89, 86, 63, 64, 80, 98, 102, 82, 81, 107, 106, 108, 109, 105, 110, 104, 103, 99, 79, 84, 100, 117, 101, 97, 115, 123, 124, 125, 120, 119, 122, 126, 121, 118, 95, 96, 112, 127, 128, 114, 113, 116, 111 ]:
 Order := 8 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T2-2,2,2-g0-path1", "8T4-4,2,2-g0-path1", "16T10-4,4,2-g1-path5", "32S9-4,8,2-g3-path3", "64S38-4,16,2-g7-path3", "128S151-8,32,2-g23-path1" ];
s`SolvableDBParents := [ Strings() | "256S373-8,32,4-g77-path2", "256S439-8,32,4-g77-path4", "256S408-8,32,4-g77-path2", "256S466-8,32,4-g77-path7", "256S408-8,32,2-g45-path2", "256S432-8,32,2-g45-path4", "256S373-8,32,2-g45-path2" ];
