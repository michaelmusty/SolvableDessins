...
I<[x]> := ideal< P | [
x[2]^2*x[5]^4 + nu^2*x[1]*x[2]^3 + (nu^3 + 1)*x[1]*x[2]^2*x[3] + x[5]^4 + (-nu^2 + nu)*x[1]*x[2]^2 + (-nu^3 - 1)*x[1]*x[2]*x[3] - nu*x[1]*x[2],
x[2]*x[3]*x[5]^4 - nu*x[2]*x[5]^4 + nu^2*x[3]*x[5]^4 + nu^2*x[1]*x[2]^2*x[3] - nu^3*x[5]^4 + x[1]*x[2]^2 - nu^2*x[1]*x[2]*x[3] - x[1]*x[2],
...
x[1]^2*x[2]*x[4] + (-nu^2 - nu)*x[1]^2*x[3]*x[4] + (-nu^2 - 2*nu - 1)*x[1]*x[4]^3 + (nu^3 + 2*nu^2 + 2*nu)*x[1]^2*x[4] + (-2*nu^3 - nu^2 + 1)*x[1]*x[5]^2 + nu^2*x[2]*x[4] + (-nu^3 + nu^2 + 2*nu + 2)*x[3]*x[4] - nu^3*x[4],
x[1]*x[2]^2*x[4] - nu^2*x[1]*x[2]*x[3]*x[4] - x[2]*x[3]*x[5]^2 - x[1]*x[2]*x[4] + nu^2*x[1]*x[3]*x[4] + nu*x[2]*x[5]^2 - nu^2*x[3]*x[5]^2 + nu^3*x[5]^2,
x[1]*x[2]*x[5]^2 + nu^2*x[2]*x[3]*x[4] + x[1]*x[5]^2 + (nu^3 + 1)*x[2]*x[4] + nu*x[3]*x[4],
x[1]*x[3]*x[5]^2 + nu^2*x[1]*x[5]^2 + nu^2*x[2]*x[4] + nu^3*x[3]*x[4],
...
s`SolvableDBName := "128S61-16,16,2-g25-path11";
s`SolvableDBFilename := "128S61-16,16,2-g25-path11.m";
...
{ IntegerRing() | 116, 128 }
@};
s`SolvableDBBelyiMapTiming := 20.850p15;
s`SolvableDBLocalSanityCheckTiming := 3.800p15;
...
\[ 6, 1, 21, 22, 19, 23, 4, 2, 3, 5, 7, 54, 55, 49, 56, 9, 50, 30, 51, 17, 44, 15, 43, 16, 18, 8, 10, 11, 12, 13, 14, 20, 24, 25, 103, 104, 97, 105, 29, 98, 67, 99, 92, 91, 31, 69, 57, 70, 90, 42, 89, 45, 46, 84, 38, 83, 39, 47, 48, 26, 27, 28, 32, 33, 34, 35, 36, 37, 40, 41, 52, 53, 58, 59, 118, 75, 108, 102, 66, 110, 111, 65, 63, 101, 68, 113, 106, 114, 60, 64, 71, 61, 100, 115, 107, 116, 73, 82, 62, 85, 93, 94, 119, 78, 72, 79, 87, 95, 96, 74, 76, 77, 80, 81, 86, 88, 109, 128, 125, 112, 126, 124, 127, 120, 122, 117, 121, 123 ]:
 Order := 16 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T2-2,2,2-g0-path1", "8T4-4,2,2-g0-path1", "16T10-4,4,2-g1-path5", "32S9-4,8,2-g3-path3", "64S6-8,8,2-g9-path9", "128S61-16,16,2-g25-path11" ];
s`SolvableDBParents := [ Strings() | "256S155-16,16,4-g81-path35", "256S368-32,32,2-g57-path9", "256S368-32,32,4-g89-path9", "256S57-16,16,4-g81-path5", "256S370-32,32,2-g57-path5", "256S370-32,32,4-g89-path5", "256S367-32,32,2-g57-path13", "256S367-32,32,4-g89-path13", "256S106-16,16,4-g81-path57", "256S369-32,32,2-g57-path5", "256S369-32,32,4-g89-path5", "256S59-16,16,4-g81-path5", "256S56-16,16,2-g49-path5", "256S90-16,16,2-g49-path16", "256S58-16,16,2-g49-path5" ];
