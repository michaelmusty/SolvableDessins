...
I<[x]> := ideal< P | [
x[2]*x[4]^6 + 1/2*(-2*nu^6 + 3*nu^4 - 1)*x[2]^2*x[4]^4 + 1/2*(nu^7 - nu)*x[1]*x[2]^4 + 1/2*(nu^6 + nu^4 + 3*nu^2 - 1)*x[2]^3*x[4]^2 + 1/2*(nu^6 - 1)*x[2]^4 + 1/2*(nu^6 + nu^4 + nu^2)*x[4]^4 + 1/2*(-nu^7 + nu)*x[1]*x[2]^2 + 1/2*(nu^6 - 2*nu^4 - 4*nu^2 - 3)*x[2]*x[4]^2 + 1/2*(-2*nu^6 - nu^4 + 2)*x[2]^2,
x[2]^2*x[3]^4 + (-nu^7 + 2*nu^5)*x[2]^2*x[3]^2 + nu^4*x[3]^4 + nu*x[1]*x[2]^2 + (-2*nu^6 - 2*nu^4 + 2*nu^2 - 2)*x[2]*x[4]^2 + (2*nu^6 - nu^2 + 2)*x[2]^2 - nu^3*x[3]^2 - nu*x[1] + nu^2,
x[2]^3*x[3]^2 + (-2*nu^3 + 2*nu)*x[2]*x[4]^4 + nu^4*x[1]*x[2]^3 + (-3*nu^7 + nu^5 + nu^3 - 3*nu)*x[2]^2*x[4]^2 + (nu^3 + nu)*x[2]^3 + nu^4*x[2]*x[3]^2 - nu^4*x[1]*x[2] + (nu^3 + nu)*x[4]^2 + (nu^7 + nu^5 - nu^3 - nu)*x[2],
x[1]^2*x[2]^2 - nu^7*x[2]^2*x[3]^2 + (-2*nu^3 + nu)*x[1]*x[2]^2 + (2*nu^6 - 2*nu^4 + 2*nu^2 - 2)*x[2]*x[4]^2 - x[1]^2 + (nu^6 - 2*nu^2 + 2)*x[2]^2 + nu^3*x[3]^2 + nu*x[1] - nu^2,
x[3]^2*x[4]^2 + nu^4*x[2]*x[3]^2 - nu^5*x[4]^2 + nu^5*x[2],
x[1]*x[3]^2 - 1,
x[1]*x[4]^2 - x[1]*x[2] + nu^3*x[4]^2 + nu^7*x[2]
] >;
...
s`SolvableDBName := "128S92-16,8,2-g21-path5";
s`SolvableDBFilename := "128S92-16,8,2-g21-path5.m";
...
{ IntegerRing() | 114, 120 }
@};
s`SolvableDBBelyiMapTiming := 11.160p15;
s`SolvableDBLocalSanityCheckTiming := 9.880p15;
...
\[ 6, 1, 21, 22, 19, 23, 4, 2, 3, 5, 7, 50, 55, 52, 56, 57, 53, 58, 54, 17, 59, 60, 61, 16, 18, 8, 9, 10, 11, 12, 13, 14, 15, 20, 24, 25, 48, 96, 30, 51, 75, 87, 83, 43, 73, 103, 104, 99, 92, 86, 63, 100, 101, 102, 38, 82, 105, 97, 106, 107, 108, 46, 47, 49, 26, 27, 28, 29, 31, 32, 33, 34, 35, 36, 37, 39, 40, 41, 42, 44, 45, 62, 64, 93, 98, 125, 109, 118, 88, 74, 81, 65, 126, 120, 122, 94, 111, 123, 127, 95, 124, 128, 68, 114, 66, 91, 71, 80, 84, 67, 69, 70, 72, 76, 77, 78, 79, 85, 89, 90, 115, 116, 110, 117, 121, 113, 112, 119 ]:
 Order := 16 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T2-2,2,2-g0-path1", "8T4-4,2,2-g0-path1", "16T13-8,2,2-g0-path1", "32S9-8,4,2-g3-path5", "64S41-16,4,2-g7-path2", "128S92-16,8,2-g21-path5" ];
s`SolvableDBParents := [ Strings() | "256S263-16,8,4-g73-path15", "256S82-16,16,2-g49-path5", "256S83-16,16,4-g81-path7", "256S101-16,8,4-g73-path39", "256S80-16,16,2-g49-path4", "256S81-16,16,4-g81-path5", "256S100-16,8,2-g41-path21" ];
