...
s`SolvableDBName := "32S5-8,4,8-g9-path11";
s`SolvableDBFilename := "32S5-8,4,8-g9-path11.m";
...
s`SolvableDBSanityCheckTiming := 2.180p15;
...
I<x1, x2, x3> := ideal< PX | [
x1^3*x3^2 + 1/2*(-2*nu - 1)*x1*x2^2*x3^2 - 1/2*x1^2*x2^2 + nu*x1*x2*x3^2 + 1/2*(-nu - 1)*x1^2*x2 - 1/2*x1*x3^2 + 1/2*(-nu - 2)*x1^2 + 1/2*(nu - 1)*x2^2 + 1/2*(nu + 1)*x2,
x1^2*x2*x3^2 + (-nu - 1)*x1^2*x3^2 + 1/2*(nu + 2)*x2^2*x3^2 - x1^3 + 1/2*(-2*nu - 1)*x1*x2^2 + (nu - 1)*x2*x3^2 - nu*x1*x2 - 1/2*nu*x3^2 - 1/2*x1,
x2^3 - 2*x1^2 + x2
] >;
...
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T2-2,2,2-g0-path1", "8T4-4,2,2-g0-path1", "16T10-4,2,4-g1-path9", "32S5-8,4,8-g9-path11" ];
