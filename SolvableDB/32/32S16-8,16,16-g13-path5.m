...
I<[x]> := ideal< P | [
x[2]*x[4]^4 - 2*x[3]*x[4]^2 - x[1]^2 + 2*x[1],
x[3]*x[4]^4 - x[1]*x[3]^3 + x[3]^3 - 1/2*x[1]*x[4]^2 + 1/2*x[2]*x[3] - 1/2*x[4]^2,
x[1]*x[2]*x[3]^2 - x[2]*x[3]^2 - 2*x[3]*x[4]^2 - x[1]^2 + x[1],
x[2]^2*x[3]^2 - 2*x[1]*x[3]^2 - x[1]*x[2],
x[1]^2*x[4]^2 - x[1]*x[2]*x[3] + x[2]*x[3] + x[4]^2,
x[1]*x[2]*x[4]^2 - x[2]^2*x[3] + x[2]*x[4]^2 + 2*x[1]*x[3],
x[1]*x[3]*x[4]^2 + x[3]*x[4]^2 - x[1],
x[2]*x[3]*x[4]^2 + x[1]*x[3]^2 - x[3]^2 - x[2],
x[1]^3 - x[2]^2 + x[1],
x[1]^2*x[3] - x[2]*x[4]^2 - x[1]*x[3]
] >;
...
s`SolvableDBName := "32S16-8,16,16-g13-path5";
s`SolvableDBFilename := "32S16-8,16,16-g13-path5.m";
...
s`SolvableDBLocalSanityCheckTiming := 1.120p15;
...
\[ 6, 1, 15, 18, 19, 22, 4, 3, 2, 5, 7, 8, 30, 25, 27, 13, 24, 26, 9, 17, 14, 10, 16, 11, 12, 20, 21, 23, 28, 32, 29, 31 ]:
 Order := 8 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,2,1-g0-path1", "4T2-2,2,2-g0-path3", "8T2-2,4,4-g1-path5", "16T5-4,8,8-g5-path5", "32S16-8,16,16-g13-path5" ];
