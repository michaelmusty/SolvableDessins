...
s`SolvableDBName := "32S8-8,8,4-g9-path3";
s`SolvableDBFilename := "32S8-8,8,4-g9-path3.m";
...
{ IntegerRing() | 31, 32 }
@};
s`SolvableDBBelyiMapTiming := 6.640p15;
s`SolvableDBSanityCheckTiming := 5.450p15;
...
x2*x4^4 + 1/2*(-nu^3 - 5*nu)*x4^4 + 8*x2^3 + (-4*nu^3 - 20*nu)*x2^2 + (4*nu^3 + 20*nu)*x3^2 - 16*x2 + 4*nu^3 + 20*nu,
x3*x4^4 + 4*x2^2*x3 - 4*x3^3 + 1/2*(-nu^3 - 7*nu)*x1*x4^2 + (-2*nu^3 - 10*nu)*x2*x3 - 4*x3,
x2^3*x3 + 1/8*(nu^3 + 7*nu)*x1*x2*x4^2 + 1/8*(nu^2 + 3)*x1*x4^2 - x2*x3,
x1*x3*x4^2 + (-nu^3 - 7*nu)*x2^2 + (nu^3 + 7*nu)*x3^2 + (-nu^2 - 3)*x2 + nu^3 + 7*nu,
x2*x3*x4^2 + 1/2*(nu^3 + 5*nu)*x3*x4^2 + (-nu^3 - 7*nu)*x1*x2,
x1*x2*x3 + 1/8*(nu^3 + 7*nu)*x2*x4^2 + 1/8*(nu^2 + 3)*x4^2,
x1*x3^2 + 1/4*(nu^3 + 7*nu)*x3*x4^2 + x1,
...
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,2,1-g0-path1", "4T2-2,2,2-g0-path3", "8T4-4,2,2-g0-path3", "16T10-4,4,2-g1-path11", "32S8-8,8,4-g9-path3" ];
s`SolvableDBParents := [ Strings() | "64S5-8,8,4-g17-path77", "64S11-8,8,4-g17-path53", "64S13-8,8,4-g17-path43", "64S13-8,8,4-g17-path44", "64S11-8,8,4-g17-path54", "64S5-8,8,4-g17-path78", "64S24-8,8,4-g17-path97" ];
