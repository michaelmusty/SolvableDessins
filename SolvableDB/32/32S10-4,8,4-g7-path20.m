...
s`SolvableDBSanityCheckTiming := 3.320p15;
...
I<x1, x2, x3> := ideal< PX | [
x1^2*x3^2 + 1/2*(nu^3 - nu + 1)*x2^2*x3^2 + (nu^3 - nu - 1)*x2*x3^2 - 1/2*x1*x2 + 1/2*(-nu^3 + nu + 1)*x3^2 + 1/2*(nu^3 - nu + 1)*x1,
x2^3 - 2*x1^2 - x2
] >;
...
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,1,2-g0-path1", "4T2-2,2,2-g0-path2", "8T4-2,4,2-g0-path2", "16T10-4,4,2-g1-path7", "32S10-4,8,4-g7-path20" ];
s`SolvableDBParents := [ Strings() | "64S7-8,8,4-g17-path17", "64S13-8,8,4-g17-path9", "64S7-8,8,4-g17-path18", "64S14-8,8,4-g17-path9", "64S9-4,8,4-g13-path25", "64S21-4,8,4-g13-path13", "64S9-4,8,4-g13-path26" ];
