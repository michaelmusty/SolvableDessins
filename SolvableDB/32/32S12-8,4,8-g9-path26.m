...
[ 22, 8, 5, 15, 7, 17, 29, 18, 13, 11, 12, 27, 30, 16, 23, 1, 20, 25, 24, 26, 4, 14, 21, 28, 2, 6, 10, 32, 3, 31, 9, 19 ],
[ 12, 28, 27, 5, 8, 7, 11, 9, 17, 32, 13, 24, 21, 10, 16, 18, 1, 19, 20, 3, 22, 2, 29, 4, 30, 14, 31, 6, 25, 26, 15, 23 ]
],
[ PermutationGroup<32 |  
\[ 2, 9, 8, 16, 11, 1, 25, 19, 21, 13, 24, 31, 26, 18, 29, 12, 3, 30, 4, 14, 5, 10, 22, 6, 28, 7, 32, 23, 27, 15, 17, 20 ],
\[ 3, 10, 14, 6, 16, 23, 1, 11, 19, 18, 25, 2, 24, 7, 17, 29, 21, 12, 30, 4, 26, 5, 20, 31, 27, 15, 8, 9, 22, 28, 32, 13 ],
...
[ 15, 29, 23, 13, 20, 30, 4, 22, 27, 3, 14, 7, 8, 21, 24, 26, 28, 5, 10, 9, 32, 17, 31, 18, 16, 19, 1, 12, 6, 11, 25, 2 ],
[ 28, 17, 9, 18, 13, 12, 30, 20, 22, 21, 4, 15, 14, 19, 25, 24, 27, 26, 5, 10, 8, 32, 2, 7, 6, 11, 23, 29, 31, 16, 1, 3 ],
[ 17, 22, 20, 24, 4, 28, 6, 5, 8, 14, 7, 1, 11, 26, 31, 15, 9, 16, 18, 19, 13, 21, 32, 12, 29, 30, 3, 2, 23, 25, 27, 10 ]
]
];
...
I<x1, x2, x3> := ideal< PX | [
x2*x3^4 + 8*x3^4 + 2*x2*x3^2 + 2*x2 - 16,
x1^4 + x2*x3^2 + 8*x3^2 + x2 - 12
] >;
X<x1, x2, x3> := Curve(AA, I);
KX<x1, x2, x3> := FunctionField(X);
s`SolvableDBBelyiCurve := X;
s`SolvableDBBelyiMap := KX!((-32*x1^8 - 128*x1^4)/(x1^12 - 12*x1^8 + 48*x1^4 - 64)*x3^2 + (x1^16 - 48*x1^12 - 672*x1^8 - 768*x1^4 + 256)/(x1^16 - 16*x1^12 + 96*x1^8 - 256*x1^4 + 256));

/*
Graph Data
*/

s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,2,1-g0-path1", "4T2-2,2,2-g0-path3", "8T2-4,2,4-g1-path5", "16T8-4,4,4-g3-path10", "32S12-8,4,8-g9-path26" ];
s`SolvableDBParents := [ Strings() | "64S16-8,8,8-g21-path65", "64S15-8,8,8-g21-path65", "64S45-16,4,16-g21-path21", "64S44-16,8,16-g25-path21", "64S45-16,4,16-g21-path22", "64S44-16,8,16-g25-path22", "64S17-8,4,8-g17-path19" ];
