...
phi := KX!((x[3]^16 - 2*x[3]^14 + 2*x[3]^10 - 2*x[3]^8 + 2*x[3]^6 - 2*x[3]^2 + 1)/(64*x[3]^8)*x[2] + (x[3]^16 - 4*x[3]^12 + 6*x[3]^8 - 4*x[3]^4 + 1)/(8*x[3]^8));
/* assign to object */
...
s`SolvableDBName := "32S3-4,8,8-g9-path1";
s`SolvableDBFilename := "32S3-4,8,8-g9-path1.m";
...
s`SolvableDBParents := [ Strings() | "64S2-8,8,8-g21-path1", "64S3-8,8,8-g21-path1", "64S26-4,16,16-g21-path6", "64S26-8,16,16-g25-path5", "64S27-4,16,16-g21-path3", "64S27-8,16,16-g25-path3", "64S17-4,8,8-g17-path12" ];
