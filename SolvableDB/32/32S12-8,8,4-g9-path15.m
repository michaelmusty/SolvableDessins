...
s`SolvableDBName := "32S12-8,8,4-g9-path15";
s`SolvableDBFilename := "32S12-8,8,4-g9-path15.m";
...
s`SolvableDBSanityCheckTiming := 15.870p15;
...
I<x1, x2, x3> := ideal< PX | [
x1^4*x3^2 - 1/2*x1^4 + 4*x3^2 - x2,
x1^4*x2 + x2^2 + 4,
x2*x3^2 - 2*x3^2 + 1/2*x2 + 1
] >;
X<x1, x2, x3> := Curve(AA, I);
KX<x1, x2, x3> := FunctionField(X);
s`SolvableDBBelyiCurve := X;
s`SolvableDBBelyiMap := KX!(-16*x2^2/(x2^4 - 8*x2^2 + 16));

/*
Graph Data
*/

s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,2,1-g0-path1", "4T1-4,4,1-g0-path1", "8T2-4,4,2-g1-path1", "16T6-8,8,2-g3-path1", "32S12-8,8,4-g9-path15" ];
s`SolvableDBParents := [ Strings() | "64S44-16,16,8-g25-path11", "64S44-16,16,8-g25-path12", "64S45-16,16,4-g21-path11", "64S16-8,8,8-g21-path99", "64S45-16,16,4-g21-path12", "64S15-8,8,8-g21-path99", "64S17-8,8,4-g17-path7" ];
s`SolvableDBChildren := [ Strings() | "16T6-8,8,2-g3-path1" ];
