...
I<[x]> := ideal< P | [
x[2]^3*x[3]^2 - nu^2*x[2]*x[3]^2 - x[2]^2 - nu^2,
x[1]^2*x[2]^2 - x[1]^2 - nu^2*x[2]^2 - nu^2,
x[1]^2*x[3]^2 + (nu^2 - 1)*x[2]^2*x[3]^2 - nu^2*x[1]^2*x[2] + nu^2*x[3]^2 - nu^2*x[2]
] >;
...
phi := KX!((-nu^2*x[3]^16 - x[3]^12 - 5*nu^2*x[3]^8 - x[3]^4)/(x[3]^16 - 2*x[3]^8 + 1)*x[2]^2 + (2*nu^2*x[3]^14 - 4*x[3]^10 + 6*nu^2*x[3]^6)/(x[3]^16 - 2*x[3]^8 + 1)*x[2] + (3*nu^2*x[3]^12 + x[3]^8 + 3*nu^2*x[3]^4 + 1)/(x[3]^16 - 2*x[3]^8 + 1));
/* assign to object */
...
s`SolvableDBLocalSanityCheckTiming := 2.030p15;
...
\[ 6, 1, 4, 22, 18, 23, 20, 24, 2, 3, 5, 7, 8, 17, 19, 21, 26, 9, 29, 27, 14, 12, 11, 16, 32, 13, 10, 30, 31, 15, 25, 28 ]:
 Order := 8 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,1,2-g0-path1", "4T2-2,2,2-g0-path2", "8T4-2,2,4-g0-path2", "16T10-4,2,4-g1-path8", "32S10-8,4,4-g7-path10" ];
s`SolvableDBParents := [ Strings() | "64S7-8,4,8-g17-path53", "64S13-8,4,8-g17-path27", "64S7-8,4,8-g17-path54", "64S14-8,4,8-g17-path27", "64S9-8,4,4-g13-path19", "64S21-8,4,4-g13-path10", "64S9-8,4,4-g13-path20" ];
