...
s`SolvableDBSanityCheckTiming := 2.140p15;
...
I<x1, x2, x3> := ideal< PX | [
x1^2*x3^2 + 1/2*(-nu^3 + nu - 1)*x2^2*x3^2 + (nu^3 - nu - 1)*x2*x3^2 - x1^2 + 1/2*(-nu^3 + nu - 1)*x2^2 + 1/2*(nu^3 - nu - 1)*x3^2 - x2 + 1/2*(-nu^3 + nu - 1),
x2^3 - 2*x1^2 - x2
] >;
...
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,2,1-g0-path1", "4T2-2,2,2-g0-path3", "8T4-2,2,4-g0-path3", "16T10-2,4,4-g1-path8", "32S10-4,8,4-g7-path10" ];
s`SolvableDBParents := [ Strings() | "64S7-4,8,8-g17-path37", "64S13-4,8,8-g17-path19", "64S7-4,8,8-g17-path38", "64S14-4,8,8-g17-path19", "64S9-4,8,4-g13-path3", "64S21-4,8,4-g13-path2", "64S9-4,8,4-g13-path4" ];
