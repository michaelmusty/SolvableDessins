...
I<[x]> := ideal< P | [
x[2]^2*x[3]^4 - x[1]^4 + 8*x[3]^4 + 12*x[1]*x[3]^2 + 3*x[1]^2,
x[1]^5 - x[2]^2*x[3]^2 - 4*x[1]^3 - 8*x[3]^2 - 9*x[1],
x[1]*x[2]^2*x[3]^2 - x[1]^4 + 8*x[1]*x[3]^2 + 4*x[1]^2 - x[2]^2 + 1,
x[1]^2*x[3]^2 - x[3]^2 - x[1]
] >;
...
\[ 6, 1, 22, 7, 2, 5, 19, 23, 3, 4, 8, 11, 29, 13, 9, 31, 24, 25, 10, 26, 27, 15, 12, 20, 32, 17, 16, 18, 30, 14, 21, 28 ]:
 Order := 4 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T2-2,2,2-g0-path1", "8T4-2,4,2-g0-path1", "16T12-2,8,4-g2-path2", "32S13-4,8,4-g7-path2" ];
s`SolvableDBParents := [ Strings() | "64S46-8,16,4-g19-path9", "64S46-4,16,8-g19-path9", "64S15-8,8,8-g21-path181", "64S46-8,16,4-g19-path10", "64S46-4,16,8-g19-path10", "64S15-8,8,8-g21-path182", "64S21-4,8,4-g13-path39" ];
