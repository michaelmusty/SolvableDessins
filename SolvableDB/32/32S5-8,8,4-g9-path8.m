...
{ IntegerRing() | 30, 31 }
@};
s`SolvableDBBelyiMapTiming := 0.720p15;
s`SolvableDBSanityCheckTiming := 1.500p15;
...
I<x1, x2, x3> := ideal< PX | [
x1^3*x3^2 + 1/2*(nu - 2)*x1*x2^2*x3^2 - 1/2*x1^2*x2^2 - nu*x1*x2*x3^2 + 1/2*(nu - 1)*x1^2*x2 - 1/2*nu*x1*x3^2 + 1/2*(nu + 2)*x1^2 + 1/2*(-nu - 1)*x2^2 + 1/2*(nu + 1)*x2,
x1^2*x2*x3^2 + (nu - 1)*x1^2*x3^2 + 1/2*(-nu - 2)*x2^2*x3^2 - x1^3 + 1/2*(nu - 2)*x1*x2^2 + (-nu - 1)*x2*x3^2 + nu*x1*x2 - 1/2*nu*x3^2 - 1/2*nu*x1,
x2^3 - 2*x1^2 - x2
] >;
