...
s`SolvableDBName := "32S15-8,8,8-g11-path4";
s`SolvableDBFilename := "32S15-8,8,8-g11-path4.m";
...
s`SolvableDBSanityCheckTiming := 0.900p15;
...
I<x1, x2, x3> := ideal< PX | [
x1^4*x3^2 + x2*x3^2 - 2*x1*x2 - 12*x3^2 - 16*x1,
x1^3*x2*x3^2 - x1^4*x2 - 8*x1^3*x3^2 + 12*x1^4 - x2^2 - 16,
x1^5 + x2*x3^2 - x1*x2 - 8*x3^2 - 12*x1,
x2*x3^4 - 2*x1*x2*x3^2 - 8*x3^4 + 2*x1^2*x2 + 16*x1^2
] >;
X<x1, x2, x3> := Curve(AA, I);
KX<x1, x2, x3> := FunctionField(X);
s`SolvableDBBelyiCurve := X;
s`SolvableDBBelyiMap := KX!(-4/x1^4*x3^4 - 8/x1^5*x3^3 - 8/x1^6*x3^2 - 4/x1^7*x3 + (x1^8 - 1)/x1^8);

/*
Graph Data
*/

s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,2,1-g0-path1", "4T2-2,2,2-g0-path3", "8T2-4,2,4-g1-path5", "16T8-4,4,4-g3-path10", "32S15-8,8,8-g11-path4" ];
s`SolvableDBParents := [ Strings() | "64S15-8,8,8-g21-path274", "64S16-8,8,8-g21-path274", "64S22-8,8,8-g21-path34" ];
