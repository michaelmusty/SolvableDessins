...
s`SolvableDBName := "32S11-8,4,4-g7-path9";
s`SolvableDBFilename := "32S11-8,4,4-g7-path9.m";
...
{ IntegerRing() | 30, 31 }
@};
s`SolvableDBBelyiMapTiming := 1.450p15;
s`SolvableDBSanityCheckTiming := 1.440p15;
...
I<x1, x2, x3> := ideal< PX | [
x1^2*x3^2 + 1/2*(nu^2 - 2)*x2^2*x3^2 - nu^2*x2*x3^2 - x1^2 + 1/2*(nu^2 - 2)*x2^2 - 1/2*nu^2*x3^2 + nu^2*x2 - 1/2*nu^2,
x2^3 - 2*x1^2 - x2
] >;
...
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,1,2-g0-path1", "4T2-2,2,2-g0-path2", "8T4-2,2,4-g0-path2", "16T10-4,2,4-g1-path7", "32S11-8,4,4-g7-path9" ];
s`SolvableDBParents := [ Strings() | "64S6-8,4,8-g17-path22", "64S11-8,4,8-g17-path44", "64S10-8,4,8-g17-path22", "64S7-8,4,8-g17-path112", "64S9-8,4,4-g13-path112", "64S8-8,4,4-g13-path22", "64S20-8,4,4-g13-path44" ];
