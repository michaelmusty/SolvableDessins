...
I<[x]> := ideal< P | [
x[2]^2*x[3]^6 + 16*x[3]^6 + x[1]^5 - 24*x[1],
x[1]^6 + x[2]^2*x[3]^4 + 16*x[3]^4 - 24*x[1]^2,
x[1]*x[3]^2 - 1
] >;
...
phi := KX!((128*x[3]^24 + 32*x[3]^16)/(256*x[3]^32 - 256*x[3]^24 + 96*x[3]^16 - 16*x[3]^8 + 1)*x[2] + 512*x[3]^16/(256*x[3]^32 - 256*x[3]^24 + 96*x[3]^16 - 16*x[3]^8 + 1));
/* assign to object */
...
s`SolvableDBName := "32S13-8,4,4-g7-path5";
s`SolvableDBFilename := "32S13-8,4,4-g7-path5.m";
...
\[ 6, 1, 8, 24, 20, 25, 22, 26, 2, 3, 4, 5, 7, 29, 30, 14, 16, 21, 28, 9, 23, 19, 32, 13, 12, 17, 15, 11, 10, 31, 18, 27 ]:
 Order := 8 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,1,2-g0-path1", "4T2-2,2,2-g0-path2", "8T2-2,4,4-g1-path4", "16T8-4,4,4-g3-path1", "32S13-8,4,4-g7-path5" ];
s`SolvableDBParents := [ Strings() | "64S46-16,8,4-g19-path1", "64S46-16,4,8-g19-path1", "64S15-8,8,8-g21-path31", "64S46-16,8,4-g19-path2", "64S46-16,4,8-g19-path2", "64S15-8,8,8-g21-path32", "64S21-8,4,4-g13-path7" ];
