...
I<[x]> := ideal< P | [
x[1]^2*x[3]^4 + 1/2*(-nu^2 - 1)*x[2]^2*x[3]^4 + (nu^2 + 1)*x[1]^3*x[3]^2 - 1/2*nu^2*x[1]^4 + 1/2*(-nu^2 - 1)*x[3]^4 - 2*nu^3*x[1]^2*x[2] + (-nu^2 + 3)*x[1]*x[3]^2 + 1/2*(-3*nu^2 + 1)*x[1]^2 + 1/2*(-nu^2 + 1)*x[2]^2 - 2*nu^3*x[2] - 3/2*nu^2,
x[2]^3*x[3]^2 - x[1]*x[2]^3 + nu^3*x[2]^2*x[3]^2 + nu^3*x[1]*x[2]^2 + x[2]*x[3]^2 + nu^2*x[1]*x[2] + nu^3*x[3]^2 + nu*x[1],
x[1]^3*x[2] - nu^3*x[1]^2*x[3]^2 + 1/2*(nu^3 - nu)*x[2]^2*x[3]^2 + 1/2*(-nu^3 + nu)*x[1]^3 + 1/2*(-nu^3 + nu)*x[1]*x[2]^2 + x[1]*x[2] + 1/2*(nu^3 - nu)*x[3]^2 - nu^3*x[1],
x[1]^2*x[2]^2 - x[1]^2 + nu^2*x[2]^2 + nu^2,
x[1]*x[2]*x[3]^2 + 1/2*(nu^2 - 1)*x[1]^2*x[2] + nu^3*x[1]*x[3]^2 + 1/2*(nu^3 + nu)*x[1]^2 + 1/2*(nu^2 - 1)*x[2] + 1/2*(nu^3 + nu)
] >;
...
s`SolvableDBLocalSanityCheckTiming := 2.110p15;
...
\[ 19, 5, 32, 23, 6, 29, 4, 22, 12, 16, 8, 1, 11, 27, 25, 21, 14, 10, 9, 31, 18, 13, 28, 3, 20, 24, 30, 7, 2, 17, 15, 26 ]:
 Order := 4 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,1,2-g0-path1", "4T2-2,2,2-g0-path2", "8T4-2,4,2-g0-path2", "16T10-4,4,2-g1-path6", "32S11-4,8,4-g7-path18" ];
s`SolvableDBParents := [ Strings() | "64S6-8,8,4-g17-path1", "64S7-8,8,4-g17-path80", "64S10-8,8,4-g17-path1", "64S11-8,8,4-g17-path12", "64S20-4,8,4-g13-path23", "64S8-4,8,4-g13-path1", "64S9-4,8,4-g13-path91" ];
