...
s`SolvableDBName := "32S10-4,8,4-g7-path3";
s`SolvableDBFilename := "32S10-4,8,4-g7-path3.m";
...
{ IntegerRing() | 30, 32 }
@};
s`SolvableDBBelyiMapTiming := 0.770p15;
s`SolvableDBSanityCheckTiming := 17.510p15;
s`SolvableDBLocalSanityCheckTiming := 1.110p15;
...
I<x1, x2, x3> := ideal< PX | [
x1^2*x2*x3^2 - 2*nu*x1^2*x3^2 - 2*x1*x2*x3^2 + 2*nu*x2^2*x3^2 - 1/4*nu*x1^2*x2 - x2*x3^2 - 1/2*x2^2 - 1/2*x1 - 1/4*nu*x2,
x1*x2^2*x3^2 - 5/2*x1^2*x3^2 - 1/2*x1^2*x2 - 1/4*nu*x1*x2^2 - 1/8*nu*x1^2 + 1/2*nu*x2^2 + 1/2*x3^2 + 1/2*nu*x1 - 1/2*x2 + 1/8*nu,
x2^3*x3^2 + 1/2*nu*x1^2*x3^2 - 2*x1*x2*x3^2 + 1/4*nu*x1^2*x2 - 1/2*x1*x2^2 - 1/4*nu*x2^3 - nu*x1*x3^2 - x2*x3^2 - 3/8*x1^2 - 1/4*nu*x1*x2 - 1/2*nu*x3^2 + 1/4*nu*x2 + 1/8,
x1^3 - 2*x2^2 - x1
] >;
...
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,2,1-g0-path1", "4T2-2,2,2-g0-path3", "8T4-2,4,2-g0-path3", "16T10-2,4,4-g1-path11", "32S10-4,8,4-g7-path3" ];
s`SolvableDBParents := [ Strings() | "64S7-4,8,8-g17-path51", "64S13-4,8,8-g17-path26", "64S7-4,8,8-g17-path52", "64S14-4,8,8-g17-path26", "64S9-4,8,4-g13-path45", "64S21-4,8,4-g13-path23", "64S9-4,8,4-g13-path46" ];
