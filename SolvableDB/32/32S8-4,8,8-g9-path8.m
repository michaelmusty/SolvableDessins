...
s`SolvableDBName := "32S8-4,8,8-g9-path8";
s`SolvableDBFilename := "32S8-4,8,8-g9-path8.m";
...
s`SolvableDBSanityCheckTiming := 3.540p15;
...
I<x1, x2, x3> := ideal< PX | [
x1^3*x3^2 - 1/2*x1^2*x2^2 + 2*x1*x2*x3^2 - 2*nu*x1^2*x2 + nu*x1*x3^2 + 5/2*x1^2 + 3/2*nu*x2^2 - 2*x2 + 1/2*nu,
x1*x2^2*x3^2 - 2*nu*x1*x2*x3^2 - x1^2*x2 + x1*x3^2 - 2*nu*x1^2 + 1/2*x2^2 + 2*nu*x2 + 1/2,
x2^3 - 2*x1^2 + x2
] >;
...
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T2-2,2,2-g0-path1", "8T4-2,2,4-g0-path1", "16T10-2,4,4-g1-path6", "32S8-4,8,8-g9-path8" ];
s`SolvableDBParents := [ Strings() | "64S11-4,8,8-g17-path63", "64S5-4,8,8-g17-path87", "64S13-4,8,8-g17-path53", "64S13-4,8,8-g17-path54", "64S5-4,8,8-g17-path88", "64S11-4,8,8-g17-path64", "64S24-4,8,8-g17-path102" ];
