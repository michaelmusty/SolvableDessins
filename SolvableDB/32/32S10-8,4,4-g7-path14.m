...
I<[x]> := ideal< P | [
x[2]^4*x[3]^2 + nu^2*x[1]^2*x[3]^2 + x[2]^2*x[3]^2 + (-nu^2 + 1)*x[1]^2*x[2] + (-2*nu^3 + 2*nu)*x[1]*x[2]^2 + (nu^2 + 1)*x[2]^3 + x[3]^2,
x[1]^3*x[3]^2 - nu^3*x[2]^3*x[3]^2 + (-nu^2 - 1)*x[1]^3*x[2] - nu^2*x[1]*x[3]^2 + nu^3*x[2]*x[3]^2 + (-2*nu^3 - 2*nu)*x[1]^2 + (-3*nu^2 + 3)*x[1]*x[2] + (-nu^3 + nu)*x[2]^2 + nu^3 - nu,
x[1]^2*x[2]^2 - x[1]^2 + nu^2*x[2]^2 + nu^2,
x[1]*x[2]*x[3]^2 - nu*x[2]^2*x[3]^2 + (-nu^2 - 1)*x[1] + (-nu^3 - nu)*x[2]
] >;
...
{ IntegerRing() | 30, 31 }
@};
s`SolvableDBBelyiMapTiming := 2.350p15;
...
\[ 6, 1, 22, 23, 20, 24, 8, 25, 2, 3, 4, 5, 7, 29, 14, 26, 11, 32, 27, 9, 30, 19, 13, 12, 17, 28, 16, 10, 31, 15, 18, 21 ]:
 Order := 8 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,2,1-g0-path1", "4T2-2,2,2-g0-path3", "8T4-2,4,2-g0-path3", "16T10-4,4,2-g1-path8", "32S10-8,4,4-g7-path14" ];
s`SolvableDBParents := [ Strings() | "64S13-8,8,4-g17-path20", "64S7-8,8,4-g17-path39", "64S14-8,8,4-g17-path20", "64S7-8,8,4-g17-path40", "64S9-8,4,4-g13-path27", "64S21-8,4,4-g13-path14", "64S9-8,4,4-g13-path28" ];
