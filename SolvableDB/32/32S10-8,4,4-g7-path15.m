...
I<[x]> := ideal< P | [
x[1]^3*x[3]^2 + (-nu^2 - 1)*x[1]^2*x[3]^2 + nu^2*x[1]*x[3]^2 + 1/2*(-nu^2 + 1)*x[1]^2 + x[1] + 1/2*(nu^2 + 1),
x[1]^2*x[2]^2 - x[1]^2 + nu^2*x[2]^2 + nu^2,
x[2]^2*x[3]^2 + 1/2*(-nu^2 - 1)*x[1]*x[2]^2 + (nu^2 - 1)*x[1]*x[3]^2 + x[3]^2 + 1/2*(nu^2 + 1)*x[1] + nu^2
] >;
...
s`SolvableDBName := "32S10-8,4,4-g7-path15";
s`SolvableDBFilename := "32S10-8,4,4-g7-path15.m";
...
s`SolvableDBLocalSanityCheckTiming := 1.250p15;
...
\[ 6, 1, 22, 23, 20, 24, 8, 25, 2, 3, 4, 5, 7, 29, 14, 26, 11, 32, 27, 9, 30, 19, 13, 12, 17, 28, 16, 10, 31, 15, 18, 21 ]:
 Order := 8 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T2-2,2,2-g0-path1", "8T4-4,2,2-g0-path1", "16T10-4,4,2-g1-path9", "32S10-8,4,4-g7-path15" ];
s`SolvableDBParents := [ Strings() | "64S13-8,8,4-g17-path21", "64S7-8,8,4-g17-path41", "64S14-8,8,4-g17-path21", "64S7-8,8,4-g17-path42", "64S9-8,4,4-g13-path29", "64S21-8,4,4-g13-path15", "64S9-8,4,4-g13-path30" ];
