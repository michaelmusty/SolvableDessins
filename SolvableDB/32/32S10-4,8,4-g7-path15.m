...
I<[x]> := ideal< P | [
x[2]^4*x[3]^2 + nu*x[1]*x[2]*x[3]^2 - x[1]*x[2]^2 + 1/2*nu^3*x[2]^3 + 1/2*x[1] + nu^3*x[2],
x[1]*x[2]^2*x[3]^2 + nu^3*x[2]*x[3]^2 + 1/2*nu^3*x[1]*x[2] + 1/2*nu^2,
x[1]^2*x[2]^2 - x[1]^2 + nu^2*x[2]^2 + nu^2,
x[1]^2*x[3]^2 + nu^3*x[1]*x[2]*x[3]^2 - nu^2*x[2]^2*x[3]^2 + 1/2*nu^3*x[1]^2*x[2] - nu^2*x[3]^2 + 1/2*nu^2*x[1]
] >;
...
phi := KX!(1/2*nu*x[3]^2*x[2]^5 + (1/2*nu^2*x[3]^4 - 1/2)*x[2]^4 + (-1/2*nu*x[3]^4 - 1/8*nu^3)/x[3]^2*x[2]^3 - 3/4*x[2]^2 + 1/8*nu^3/x[3]^2*x[2] + (x[3]^4 - 1/32*nu^2)/x[3]^4);
/* assign to object */
...
s`SolvableDBName := "32S10-4,8,4-g7-path15";
s`SolvableDBFilename := "32S10-4,8,4-g7-path15.m";
...
s`SolvableDBLocalSanityCheckTiming := 2.240p15;
...
\[ 20, 5, 25, 24, 6, 30, 4, 23, 12, 16, 8, 1, 11, 29, 14, 22, 7, 27, 26, 9, 31, 28, 13, 17, 19, 3, 21, 10, 32, 2, 18, 15 ]:
 Order := 4 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T2-2,2,2-g0-path1", "8T4-4,2,2-g0-path1", "16T10-4,4,2-g1-path9", "32S10-4,8,4-g7-path15" ];
s`SolvableDBParents := [ Strings() | "64S7-8,8,4-g17-path9", "64S13-8,8,4-g17-path5", "64S7-8,8,4-g17-path10", "64S14-8,8,4-g17-path5", "64S9-4,8,4-g13-path13", "64S21-4,8,4-g13-path7", "64S9-4,8,4-g13-path14" ];
