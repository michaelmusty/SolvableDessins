...
s`SolvableDBName := "32S15-8,8,8-g11-path57";
s`SolvableDBFilename := "32S15-8,8,8-g11-path57.m";
...
s`SolvableDBSanityCheckTiming := 1.420p15;
...
I<x1, x2, x3> := ideal< PX | [
x1^3*x2*x3^2 + 7*x1*x2*x3^2 + 16*x1^3 - x2^2 + 16*x1,
x1*x2*x3^4 + 12*x1^3*x3^2 - 2*x1^2*x2 + 4*x1*x3^2 + 2*x2,
x2^2*x3^4 + 14*x1^2*x2*x3^2 - 2*x1*x2^2 + 18*x2*x3^2 + 32*x1^2 + 32,
x1^4 - 1/2*x2*x3^2 - 1
] >;
...
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,2,1-g0-path1", "4T2-2,2,2-g0-path3", "8T2-2,4,4-g1-path5", "16T8-4,4,4-g3-path5", "32S15-8,8,8-g11-path57" ];
s`SolvableDBParents := [ Strings() | "64S22-8,8,8-g21-path53", "64S16-8,8,8-g21-path293", "64S15-8,8,8-g21-path293" ];
s`SolvableDBChildren := [ Strings() | "16T8-4,4,4-g3-path5" ];
