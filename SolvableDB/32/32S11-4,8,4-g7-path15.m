...
x[1]^2*x[3]^2 + nu^2*x[1]^2*x[2] - x[3]^2 + nu^2*x[2],
x[2]^2*x[3]^2 - x[2]^3 + nu^2*x[3]^2 + nu^2*x[2]
] >;
...
phi := KX!((1/4*nu^2*x[3]^4 + 1/4)/x[3]^4*x[2]^2 - 1/2*nu^2*x[3]^2*x[2] + 1/4*nu^2*x[3]^4 + 3/4);
/* assign to object */
...
s`SolvableDBName := "32S11-4,8,4-g7-path15";
s`SolvableDBFilename := "32S11-4,8,4-g7-path15.m";
...
{ IntegerRing() | 30, 31 }
@};
s`SolvableDBBelyiMapTiming := 1.300p15;
s`SolvableDBLocalSanityCheckTiming := 3.820p15;
...
\[ 19, 5, 32, 23, 6, 29, 4, 22, 12, 16, 8, 1, 11, 27, 25, 21, 14, 10, 9, 31, 18, 13, 28, 3, 20, 24, 30, 7, 2, 17, 15, 26 ]:
 Order := 4 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T2-2,2,2-g0-path1", "8T4-4,2,2-g0-path1", "16T10-4,4,2-g1-path9", "32S11-4,8,4-g7-path15" ];
s`SolvableDBParents := [ Strings() | "64S6-8,8,4-g17-path10", "64S7-8,8,4-g17-path89", "64S10-8,8,4-g17-path10", "64S11-8,8,4-g17-path21", "64S20-4,8,4-g13-path43", "64S8-4,8,4-g13-path21", "64S9-4,8,4-g13-path111" ];
