...
s`SolvableDBName := "32S8-4,8,8-g9-path10";
s`SolvableDBFilename := "32S8-4,8,8-g9-path10.m";
...
{ IntegerRing() | 31, 32 }
@};
s`SolvableDBBelyiMapTiming := 1.570p15;
s`SolvableDBSanityCheckTiming := 3.840p15;
s`SolvableDBLocalSanityCheckTiming := 0.470p15;
...
I<x1, x2, x3> := ideal< PX | [
x1^3*x3^2 - 1/2*nu^2*x1*x2^2*x3^2 - 1/2*x1^2*x2^2 + (-nu^3 + nu - 1)*x1*x2*x3^2 - 1/2*nu^2*x1^2*x2 + 1/2*(-2*nu^3 + 3*nu^2 - 2*nu)*x1*x3^2 + 1/2*(-2*nu^3 + 2*nu + 3)*x1^2 + 1/4*(2*nu^3 + 3*nu^2 + 2*nu)*x2^2 + 1/4*(-2*nu^3 - 3*nu^2 - 2*nu),
x1^2*x2*x3^2 - nu^2*x1^2*x3^2 + (-nu^3 + nu - 1)*x2^2*x3^2 - x1^3 - 1/2*nu^2*x1*x2^2 + (-nu^3 + nu^2 - nu)*x2*x3^2 + (-nu^3 + nu + 1)*x1*x2 + 1/2*(2*nu^3 + 3*nu^2 + 2*nu)*x1,
x2^3 - 2*x1^2 - x2
] >;
...
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,2,1-g0-path1", "4T2-2,2,2-g0-path3", "8T4-2,2,4-g0-path3", "16T10-2,4,4-g1-path8", "32S8-4,8,8-g9-path10" ];
s`SolvableDBParents := [ Strings() | "64S11-4,8,8-g17-path47", "64S5-4,8,8-g17-path71", "64S13-4,8,8-g17-path37", "64S13-4,8,8-g17-path38", "64S5-4,8,8-g17-path72", "64S11-4,8,8-g17-path48", "64S24-4,8,8-g17-path94" ];
