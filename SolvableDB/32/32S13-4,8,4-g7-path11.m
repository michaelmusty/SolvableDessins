...
/* Belyi curve */
K := Rationals();
X<[x]> := HyperellipticCurve([Polynomial(K, [ -16, 0, 0, 0, 0, 0, 0, 0, 24, 0, 0, 0, 0, 0, 0, 0, -1 ]), Polynomial(K, [])]);

/* Belyi map */
K := Rationals();
X<[x]> := HyperellipticCurve([Polynomial(K, [ -16, 0, 0, 0, 0, 0, 0, 0, 24, 0, 0, 0, 0, 0, 0, 0, -1 ]), Polynomial(K, [])]);
KX<[x]> := FunctionField(X);
phi := KX!((32*x[1]^16 + 128*x[1]^8)/(x[1]^32 - 16*x[1]^24 + 96*x[1]^16 - 256*x[1]^8 + 256)*x[2] + (x[1]^32 - 16*x[1]^24 - 416*x[1]^16 - 256*x[1]^8 + 256)/(x[1]^32 - 16*x[1]^24 + 96*x[1]^16 - 256*x[1]^8 + 256));
/* assign to object */
...
s`SolvableDBName := "32S13-4,8,4-g7-path11";
s`SolvableDBFilename := "32S13-4,8,4-g7-path11.m";
...
\[ 6, 1, 24, 25, 22, 9, 4, 23, 2, 3, 5, 7, 8, 15, 32, 27, 13, 28, 11, 26, 31, 19, 17, 29, 12, 16, 20, 21, 10, 14, 18, 30 ]:
 Order := 4 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,2,1-g0-path1", "4T2-2,2,2-g0-path3", "8T2-4,2,4-g1-path5", "16T8-4,4,4-g3-path6", "32S13-4,8,4-g7-path11" ];
