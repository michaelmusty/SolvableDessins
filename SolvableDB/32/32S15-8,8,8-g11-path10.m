...
s`SolvableDBName := "32S15-8,8,8-g11-path10";
s`SolvableDBFilename := "32S15-8,8,8-g11-path10.m";
...
s`SolvableDBSanityCheckTiming := 9.560p15;
...
I<x1, x2, x3> := ideal< PX | [
x1^2*x2^2*x3^4 + 2*x2^2*x3^4 + 3*x1*x2*x3^2 + x1^2 - x2^2 - 1,
x2^2*x3^6 + 3*x1*x2*x3^4 + 3*x1^2*x3^2 - x1*x2 + x3^2,
x1^3 - x2*x3^2 - x1
] >;
X<x1, x2, x3> := Curve(AA, I);
KX<x1, x2, x3> := FunctionField(X);
s`SolvableDBBelyiCurve := X;
s`SolvableDBBelyiMap := KX!(-1/4/x1^2*x3^6 + (1/2*x1^4 - 1/2)/x1^4*x3^4 - 1/4*x1^2*x3^2 + 1/2);

/*
Graph Data
*/

s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T2-2,2,2-g0-path1", "8T2-4,4,2-g1-path3", "16T8-4,4,4-g3-path13", "32S15-8,8,8-g11-path10" ];
s`SolvableDBParents := [ Strings() | "64S15-8,8,8-g21-path242", "64S16-8,8,8-g21-path242", "64S22-8,8,8-g21-path2" ];
