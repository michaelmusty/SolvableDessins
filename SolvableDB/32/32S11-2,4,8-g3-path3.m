...
s`SolvableDBName := "32S11-2,4,8-g3-path3";
s`SolvableDBFilename := "32S11-2,4,8-g3-path3.m";
...
s`SolvableDBBelyiMap := KX!((-16769025/16384*x1^16 - 8190*nu*x1^15 + 61433/2*x1^14 + 143353/2*nu*x1^13 - 1863645/16*x1^12 - 1118201/8*nu*x1^11 + 4100089/32*x1^10 + 2928639/32*nu*x1^9 - 26357759/512*x1^8 - 22880*nu*x1^7 + 8008*x1^6 + 2184*nu*x1^5 - 455*x1^4 - 70*nu*x1^3 + 15/2*x1^2 + 1/2*nu*x1 - 1/64)/(x1^16 + 4*nu*x1^15 - 7*x1^14 - 7*nu*x1^13 + 35/8*x1^12 + 7/4*nu*x1^11 - 7/16*x1^10 - 1/16*nu*x1^9 + 1/256*x1^8));

/*
Graph Data
*/

s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,2,1-g0-path1", "4T2-2,2,2-g0-path3", "8T4-2,4,2-g0-path3", "16T10-2,4,4-g1-path11", "32S11-2,4,8-g3-path3" ];
s`SolvableDBParents := [ Strings() | "64S20-4,4,8-g13-path5", "64S6-2,8,8-g9-path5", "64S7-4,8,8-g17-path73", "64S9-4,4,8-g13-path73", "64S10-2,8,8-g9-path5", "64S11-4,8,8-g17-path5", "64S8-2,4,8-g5-path5" ];
