...
I<[x]> := ideal< P | [
x[1]^3*x[3]^2 + (-nu^2 - 1)*x[1]^2*x[3]^2 + nu^2*x[1]*x[3]^2 - nu*x[1]^2 + (-nu^3 - nu)*x[1] - nu^3,
x[1]^2*x[2]^2 - x[1]^2 + nu^2*x[2]^2 + nu^2,
x[2]^2*x[3]^2 + nu^3*x[1]*x[2]^2 + (nu^2 - 1)*x[1]*x[3]^2 + x[3]^2 - nu^3*x[1] - nu^3 + nu
] >;
...
s`SolvableDBName := "32S11-8,4,4-g7-path15";
s`SolvableDBFilename := "32S11-8,4,4-g7-path15.m";
...
\[ 6, 1, 21, 22, 19, 23, 8, 24, 2, 3, 4, 5, 7, 27, 32, 31, 14, 10, 9, 30, 26, 13, 12, 28, 16, 25, 29, 11, 15, 17, 18, 20 ]:
 Order := 8 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T2-2,2,2-g0-path1", "8T4-4,2,2-g0-path1", "16T10-4,4,2-g1-path9", "32S11-8,4,4-g7-path15" ];
s`SolvableDBParents := [ Strings() | "64S6-8,8,4-g17-path15", "64S11-8,8,4-g17-path37", "64S10-8,8,4-g17-path15", "64S7-8,8,4-g17-path105", "64S9-8,4,4-g13-path96", "64S8-8,4,4-g13-path6", "64S20-8,4,4-g13-path28" ];
