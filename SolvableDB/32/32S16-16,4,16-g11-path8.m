...
I<[x]> := ideal< P | [
x[2]^2*x[3]^4 - 3*x[1]^2*x[2]*x[3]^2 + 2*x[2]*x[3]^4 - 6*x[1]^2*x[3]^2 + 2*x[3]^4 + x[1]^2*x[2] - x[2]*x[3]^2 + 2*x[1]^2 - x[1]*x[2] - 2*x[3]^2 - 2*x[1],
x[1]^3*x[3]^2 + x[1]^2*x[3]^2 + x[1]*x[3]^2 - x[1]^2 + x[3]^2 + x[1],
x[1]*x[2]^2*x[3]^2 + 2*x[1]*x[2]*x[3]^2 + 2*x[2]^2*x[3]^2 - 3*x[1]^2*x[2] + 2*x[1]*x[3]^2 + 4*x[2]*x[3]^2 - 6*x[1]^2 + 2*x[1]*x[2] - x[2]^2 + 4*x[3]^2 + 4*x[1] - x[2],
x[1]^3*x[2] + x[2]^2*x[3]^2 + 2*x[1]^3 - 2*x[1]^2*x[2] + 2*x[2]*x[3]^2 - 4*x[1]^2 + x[1]*x[2] + 2*x[3]^2 + 2*x[1]
] >;
...
phi := KX!((-x[2]^4 - 4*x[2]^3 - 4*x[2]^2)/(4*x[2]^2 + 8*x[2] + 4));
/* assign to object */
...
s`SolvableDBName := "32S16-16,4,16-g11-path8";
s`SolvableDBFilename := "32S16-16,4,16-g11-path8.m";
...
s`SolvableDBLocalSanityCheckTiming := 1.240p15;
...
\[ 6, 1, 4, 16, 13, 17, 15, 7, 2, 3, 5, 8, 14, 25, 23, 26, 27, 11, 9, 10, 12, 18, 24, 32, 29, 30, 31, 20, 19, 21, 22, 28 ]:
 Order := 16 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,2,1-g0-path1", "4T2-2,2,2-g0-path3", "8T2-4,2,4-g1-path5", "16T5-8,2,8-g3-path3", "32S16-16,4,16-g11-path8" ];
