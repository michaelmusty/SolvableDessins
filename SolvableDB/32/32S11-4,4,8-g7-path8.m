...
s`SolvableDBSanityCheckTiming := 5.300p15;
...
I<x1, x2, x3> := ideal< PX | [
x1^2*x3^2 + nu*x1*x2*x3^2 - 1/2*x2^2*x3^2 + nu*x1*x3^2 + 1/4*(-nu - 1)*x1*x2 + 1/4*(nu - 1)*x2^2 - 1/2*x3^2 + 1/4*(nu + 1)*x1 + 1/4*(nu - 1)*x2,
x2^3 - 2*x1^2 + x2
] >;
...
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T2-2,2,2-g0-path1", "8T4-2,2,4-g0-path1", "16T10-2,4,4-g1-path6", "32S11-4,4,8-g7-path8" ];
s`SolvableDBParents := [ Strings() | "64S6-4,8,8-g17-path10", "64S11-4,8,8-g17-path32", "64S10-4,8,8-g17-path10", "64S7-4,8,8-g17-path100", "64S9-4,4,8-g13-path111", "64S8-4,4,8-g13-path21", "64S20-4,4,8-g13-path43" ];
