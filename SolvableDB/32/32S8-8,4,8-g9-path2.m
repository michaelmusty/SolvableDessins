...
{ IntegerRing() | 31, 32 }
@};
s`SolvableDBBelyiMapTiming := 1.310p15;
s`SolvableDBSanityCheckTiming := 4.470p15;
s`SolvableDBLocalSanityCheckTiming := 1.150p15;
...
I<x1, x2, x3> := ideal< PX | [
x1^3*x3^2 + 1/2*(nu - 1)*x1^2*x2*x3^2 - 1/2*nu*x1^2*x2^2 + 1/2*(-nu - 1)*x1^2*x3^2 + 2*x1*x2*x3^2 + (nu - 1)*x2^2*x3^2 + 1/2*(-nu - 1)*x1^3 + 3/2*x1^2*x2 + (-nu + 1)*x1*x2^2 + nu*x1*x3^2 + 1/2*(-3*nu - 3)*x2*x3^2 + (nu + 1)*x1*x2 + 1/2*x2^2 + 1/2*(-nu + 1)*x3^2 + 1/2*(-nu + 1)*x1 - 1/2*nu*x2,
x1*x2^2*x3^2 + (nu - 1)*x1^2*x3^2 - 2*nu*x1*x2*x3^2 + 1/2*(nu + 1)*x2^2*x3^2 - nu*x1^2*x2 + 1/2*(-nu - 1)*x1*x2^2 + x1*x3^2 + (nu - 1)*x2*x3^2 + x1^2 + (-nu + 1)*x1*x2 - nu*x2^2 + 1/2*(-nu - 1)*x3^2 + 1/2*(-nu - 1)*x1 - x2,
x2^3 - 2*x1^2 + x2
] >;
