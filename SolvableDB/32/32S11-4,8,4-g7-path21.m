...
s`SolvableDBSanityCheckTiming := 29.940p15;
...
x3^2*x4^2 - x1*x3^2 - nu*x4^2 - x1,
x2*x3^2 - x3^2 - x2 - 1,
x1*x4^2 + (-nu - 1)*x2^2 + (nu - 1)*x2,
x2*x4^2 + (-nu - 1)*x1*x2 + nu*x4^2,
x1^2 - x2^2 - 1
] >;
X<x1, x2, x3, x4> := Curve(AA, I);
KX<x1, x2, x3, x4> := FunctionField(X);
s`SolvableDBBelyiCurve := X;
s`SolvableDBBelyiMap := KX!((-1/16*nu*x4^8 + 1/4*nu*x4^4 - 1/4*nu)/x4^4*x3^4 + (1/8*(nu - 1)*x4^8 + 1/2*(-nu + 1))/x4^4*x3^2 + (1/16*(nu + 2)*x4^8 + 1/4*(nu + 2)*x4^4 + 1/4*(nu + 2))/x4^4);

/*
Graph Data
*/

s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,2,1-g0-path1", "4T2-2,2,2-g0-path3", "8T4-2,4,2-g0-path3", "16T10-4,4,2-g1-path8", "32S11-4,8,4-g7-path21" ];
s`SolvableDBParents := [ Strings() | "64S6-8,8,4-g17-path10", "64S7-8,8,4-g17-path89", "64S10-8,8,4-g17-path10", "64S11-8,8,4-g17-path21", "64S20-4,8,4-g13-path36", "64S8-4,8,4-g13-path14", "64S9-4,8,4-g13-path104" ];
