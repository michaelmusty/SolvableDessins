...
phi := KX!((-32*x[1]^16 - 128*x[1]^8)/(x[1]^32 - 16*x[1]^24 + 96*x[1]^16 - 256*x[1]^8 + 256)*x[2] + 512*x[1]^16/(x[1]^32 - 16*x[1]^24 + 96*x[1]^16 - 256*x[1]^8 + 256));
/* assign to object */
...
s`SolvableDBName := "32S14-8,4,4-g7-path14";
s`SolvableDBFilename := "32S14-8,4,4-g7-path14.m";
...
\[ 6, 1, 8, 16, 19, 22, 4, 23, 2, 3, 5, 7, 18, 21, 13, 28, 25, 31, 9, 17, 24, 11, 26, 10, 12, 14, 15, 20, 32, 29, 30, 27 ]:
 Order := 8 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,1,2-g0-path1", "4T2-2,2,2-g0-path2", "8T2-2,4,4-g1-path4", "16T8-4,4,4-g3-path1", "32S14-8,4,4-g7-path14" ];
s`SolvableDBParents := [ Strings() | "64S16-8,8,8-g21-path227", "64S16-8,8,8-g21-path228", "64S47-16,4,4-g15-path11", "64S49-16,8,8-g23-path21", "64S48-16,4,4-g15-path11", "64S49-16,8,8-g23-path22", "64S21-8,4,4-g13-path62" ];
