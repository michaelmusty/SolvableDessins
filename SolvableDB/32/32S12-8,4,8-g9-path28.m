...
I<[x]> := ideal< P | [
x[2]^2*x[3]^6 - 24*x[1]^2*x[3]^2 - x[1]^2*x[2] - 4*x[3]^2 + 4*x[2],
x[1]*x[2]*x[3]^4 + 5*x[1]^2*x[3]^2 + x[3]^2 - x[2],
x[1]^3 - x[2]*x[3]^2 + x[1]
] >;
...
phi := KX!(-x[1]^2/16*x[3]^6 + 1/(16*x[1])*x[3]^5 + 3*x[1]/8*x[3]^3 + x[1]^3/16*x[3]);
/* assign to object */
...
\[ 6, 1, 23, 13, 21, 24, 4, 22, 2, 3, 5, 7, 8, 20, 30, 26, 28, 14, 10, 32, 9, 17, 31, 11, 16, 19, 29, 12, 15, 18, 25, 27 ]:
 Order := 8 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,1,2-g0-path1", "4T2-2,2,2-g0-path2", "8T2-4,2,4-g1-path4", "16T8-4,4,4-g3-path9", "32S12-8,4,8-g9-path28" ];
s`SolvableDBParents := [ Strings() | "64S16-8,8,8-g21-path48", "64S15-8,8,8-g21-path48", "64S45-16,4,16-g21-path1", "64S44-16,8,16-g25-path1", "64S45-16,4,16-g21-path2", "64S44-16,8,16-g25-path2", "64S17-8,4,8-g17-path2" ];
