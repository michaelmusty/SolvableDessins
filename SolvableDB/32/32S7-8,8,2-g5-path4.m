...
I<[x]> := ideal< P | [
nu^2*x[2]^4 + x[2]^2*x[3]^4 - nu^2*x[2]^2 + x[3]^4,
x[1]^2*x[2]^2 - x[1]^2 + nu^2*x[2]^2 + nu^2,
x[1]*x[2]^3 - x[1]*x[2] - x[2]^2*x[3]^2 - x[3]^2,
x[1]*x[3]^2 + nu^2*x[2]
] >;
...
phi := KX!((-x[2]^6 - nu^2*x[2]^2)/(x[2]^8 - 2*nu^2*x[2]^4 - 1)*x[3]^2 + x[2]^4/(x[2]^4 - nu^2));
/* assign to object */
...
s`SolvableDBName := "32S7-8,8,2-g5-path4";
s`SolvableDBFilename := "32S7-8,8,2-g5-path4.m";
s`SolvableDBPassportName := "32S7-8,8,2-g5";
...
{ IntegerRing() | 27, 31 }
@};
s`SolvableDBBelyiMapTiming := 0.910000000000000p15;
s`SolvableDBSanityCheckTiming := 0.870000000000000p15;
...
\[ 6, 1, 20, 21, 18, 22, 4, 2, 3, 5, 7, 32, 23, 30, 28, 25, 31, 8, 16, 15, 29, 10, 17, 9, 11, 12, 13, 14, 19, 24, 26, 27 ]:
 Order := 8 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T2-2,2,2-g0-path1", "8T4-4,2,2-g0-path1", "16T10-4,4,2-g1-path9", "32S7-8,8,2-g5-path4" ];
s`SolvableDBParents := [ Strings() | "64S4-8,8,4-g17-path45", "64S24-8,8,4-g17-path92", "64S10-8,8,4-g17-path33", "64S14-8,8,4-g17-path45", "64S4-8,8,2-g9-path45", "64S12-8,8,2-g9-path11", "64S10-8,8,2-g9-path33" ];
