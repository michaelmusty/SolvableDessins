...
s`SolvableDBName := "32S15-8,8,8-g11-path28";
s`SolvableDBFilename := "32S15-8,8,8-g11-path28.m";
...
s`SolvableDBSanityCheckTiming := 8.730p15;
s`SolvableDBLocalSanityCheckTiming := 1.210p15;
...
I<x1, x2, x3> := ideal< PX | [
x1^4*x3^2 - x1*x2 + 16*x3^2,
x1^3*x2*x3^2 - 32*nu*x1^4 - x2^2 + 512*nu,
x1^5 - 1/2*nu*x2*x3^2 - 16*x1,
x2*x3^4 + 2*nu*x1^2*x2 - 64*nu*x1*x3^2
] >;
X<x1, x2, x3> := Curve(AA, I);
KX<x1, x2, x3> := FunctionField(X);
s`SolvableDBBelyiCurve := X;
s`SolvableDBBelyiMap := KX!(-4*x1^4*x3^4);

/*
Graph Data
*/

s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,2,1-g0-path1", "4T2-2,2,2-g0-path3", "8T4-2,4,2-g0-path3", "16T8-4,4,4-g3-path21", "32S15-8,8,8-g11-path28" ];
s`SolvableDBParents := [ Strings() | "64S15-8,8,8-g21-path261", "64S16-8,8,8-g21-path261", "64S22-8,8,8-g21-path21" ];
