...
phi := KX!((-32*x[1]^7 - 128*x[1]^3)/(x[1]^12 - 12*x[1]^8 + 48*x[1]^4 - 64)*x[3]^2 + (x[1]^12 + 4*x[1]^10 - 4*x[1]^8 + 96*x[1]^6 - 16*x[1]^4 + 64*x[1]^2 + 64)/(x[1]^12 + 4*x[1]^10 - 4*x[1]^8 - 32*x[1]^6 - 16*x[1]^4 + 64*x[1]^2 + 64));
/* assign to object */
...
s`SolvableDBLocalSanityCheckTiming := 0.650p15;
...
\[ 6, 1, 23, 13, 21, 24, 4, 22, 2, 3, 5, 7, 8, 15, 30, 26, 29, 27, 10, 32, 9, 17, 31, 11, 16, 19, 20, 14, 12, 18, 25, 28 ]:
 Order := 8 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T2-2,2,2-g0-path1", "8T2-4,2,4-g1-path3", "16T8-4,4,4-g3-path8", "32S15-8,8,8-g11-path22" ];
s`SolvableDBParents := [ Strings() | "64S15-8,8,8-g21-path305", "64S16-8,8,8-g21-path305", "64S22-8,8,8-g21-path65" ];
s`SolvableDBChild := "16T8-4,4,4-g3-path8";
