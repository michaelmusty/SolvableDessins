...
I<[x]> := ideal< P | [
x[1]^3*x[2]*x[3]^2 + 7*x[1]*x[2]*x[3]^2 - 32/3*x[1]^3 + 4/3*x[2]^2 - 32/3*x[1],
x[1]*x[2]*x[3]^4 - 8*x[1]^3*x[3]^2 - 16/9*x[1]^2*x[2] - 8/3*x[1]*x[3]^2 + 16/9*x[2],
x[2]^2*x[3]^4 - 28/3*x[1]^2*x[2]*x[3]^2 - 16/9*x[1]*x[2]^2 - 12*x[2]*x[3]^2 + 128/9*x[1]^2 + 128/9,
x[1]^4 + 3/4*x[2]*x[3]^2 - 1
] >;
...
s`SolvableDBLocalSanityCheckTiming := 0.680p15;
...
\[ 10, 13, 18, 21, 2, 5, 23, 27, 6, 9, 3, 16, 20, 26, 4, 28, 30, 7, 22, 1, 11, 12, 15, 19, 32, 25, 24, 8, 14, 29, 17, 31 ]:
 Order := 8 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,2,1-g0-path1", "4T2-2,2,2-g0-path3", "8T2-4,4,2-g1-path5", "16T8-4,4,4-g3-path12", "32S15-8,8,8-g11-path11" ];
s`SolvableDBParents := [ Strings() | "64S22-8,8,8-g21-path46", "64S16-8,8,8-g21-path286", "64S15-8,8,8-g21-path286" ];
s`SolvableDBChild := "16T8-4,4,4-g3-path12";
