...
I<[x]> := ideal< P | [
x[1]*x[2]*x[3]^6 - 8*x[1]*x[3]^6 + 1/20*x[2]^2*x[3]^4 + 4/5*x[1]^3*x[3]^2 - x[2]*x[3]^4 - 56/5*x[3]^4 + 1/10*x[1]^2*x[2] + 4/5*x[1]^2,
x[1]^2*x[2]*x[3]^4 - 8*x[1]^2*x[3]^4 - 2*x[1]*x[2]*x[3]^2 + 2*x[2] + 16,
x[1]*x[2]^2*x[3]^4 + 4*x[1]*x[2]*x[3]^4 - 96*x[1]*x[3]^4 + 2*x[1]^3*x[2] + 16*x[1]^3 - 24*x[2]*x[3]^2 - 128*x[3]^2,
x[2]^3*x[3]^4 - 24*x[1]^3*x[2]*x[3]^2 - 8*x[2]^2*x[3]^4 - 128*x[1]^3*x[3]^2 + 16*x[2]*x[3]^4 + 2*x[1]^2*x[2]^2 - 128*x[3]^4 + 40*x[1]^2*x[2] + 192*x[1]^2,
x[1]^4 + x[1]*x[2]*x[3]^2 - 8*x[1]*x[3]^2 - x[2] - 12
] >;
...
phi := KX!((32*x[1]^9 + 128*x[1]^5)/(x[1]^12 - 12*x[1]^8 + 48*x[1]^4 - 64)*x[3]^2 - 32*x[1]^4/(x[1]^8 - 8*x[1]^4 + 16));
/* assign to object */
...
s`SolvableDBName := "32S15-8,8,8-g11-path18";
s`SolvableDBFilename := "32S15-8,8,8-g11-path18.m";
...
\[ 6, 1, 8, 23, 20, 24, 21, 25, 2, 3, 4, 5, 7, 28, 22, 14, 16, 27, 26, 9, 19, 30, 13, 12, 17, 11, 31, 10, 15, 18, 32, 29 ]:
 Order := 8 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,1,2-g0-path1", "4T2-2,2,2-g0-path2", "8T2-2,4,4-g1-path4", "16T8-4,4,4-g3-path1", "32S15-8,8,8-g11-path18" ];
s`SolvableDBParents := [ Strings() | "64S15-8,8,8-g21-path38", "64S16-8,8,8-g21-path38", "64S22-8,8,8-g21-path2" ];
