...
I<[x]> := ideal< P | [
x[1]^3*x[2]*x[3]^2 - 7*x[1]*x[2]*x[3]^2 - 8*x[1]^3 - x[2]^2 + 8*x[1],
x[1]*x[2]*x[3]^4 + 6*x[1]^3*x[3]^2 + x[1]^2*x[2] - 2*x[1]*x[3]^2 + x[2],
x[2]^2*x[3]^4 + 7*x[1]^2*x[2]*x[3]^2 + x[1]*x[2]^2 - 9*x[2]*x[3]^2 - 8*x[1]^2 + 8,
x[1]^4 + x[2]*x[3]^2 - 1
] >;
...
\[ 6, 1, 23, 13, 21, 24, 4, 22, 2, 3, 5, 7, 8, 15, 30, 26, 29, 27, 10, 32, 9, 17, 31, 11, 16, 19, 20, 14, 12, 18, 25, 28 ]:
 Order := 8 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T2-2,2,2-g0-path1", "8T4-2,4,2-g0-path1", "16T8-4,4,4-g3-path20", "32S15-8,8,8-g11-path18" ];
s`SolvableDBParents := [ Strings() | "64S15-8,8,8-g21-path304", "64S16-8,8,8-g21-path304", "64S22-8,8,8-g21-path64" ];
