...
I<[x]> := ideal< P | [
x[1]^4*x[2] - x[1]^4 + x[2]^2 + x[2],
x[1]^2*x[3]^2 - x[2]*x[3]^2 - 1/2*x[1]^2 - 1/2*x[2],
x[2]^2*x[3]^2 + x[1]^2*x[2] - x[1]^2 + 1/2*x[2]^2 + x[3]^2 - x[2] - 1/2
] >;
...
phi := KX!((-128*x[3]^8 - 32*x[3]^4)/(64*x[3]^12 - 64*x[3]^10 - 16*x[3]^8 + 32*x[3]^6 - 4*x[3]^4 - 4*x[3]^2 + 1)*x[2] + (64*x[3]^12 + 64*x[3]^10 - 16*x[3]^8 + 96*x[3]^6 - 4*x[3]^4 + 4*x[3]^2 + 1)/(64*x[3]^12 + 64*x[3]^10 - 16*x[3]^8 - 32*x[3]^6 - 4*x[3]^4 + 4*x[3]^2 + 1));
/* assign to object */
...
s`SolvableDBName := "32S12-8,4,8-g9-path19";
s`SolvableDBFilename := "32S12-8,4,8-g9-path19.m";
...
\[ 23, 31, 16, 20, 15, 28, 24, 7, 25, 17, 30, 22, 12, 3, 32, 13, 4, 9, 27, 8, 11, 19, 21, 29, 1, 5, 14, 26, 10, 18, 6, 2 ]:
 Order := 8 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T2-2,2,2-g0-path1", "8T2-4,2,4-g1-path3", "16T6-8,2,8-g3-path3", "32S12-8,4,8-g9-path19" ];
s`SolvableDBParents := [ Strings() | "64S16-8,8,8-g21-path65", "64S15-8,8,8-g21-path65", "64S45-16,4,16-g21-path13", "64S44-16,8,16-g25-path13", "64S45-16,4,16-g21-path14", "64S44-16,8,16-g25-path14", "64S17-8,4,8-g17-path19" ];
