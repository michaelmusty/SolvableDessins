...
s`SolvableDBName := "32S7-8,8,2-g5-path3";
s`SolvableDBFilename := "32S7-8,8,2-g5-path3.m";
...
s`SolvableDBSanityCheckTiming := 5.340p15;
s`SolvableDBLocalSanityCheckTiming := 0.980p15;
...
x2^2*x4^2 + 1/2*(nu^3 + 5*nu)*x1*x2*x3 - x1*x3 - x4^2,
x3^2*x4^2 + 1/2*(nu^3 + 5*nu)*x2*x4^2 - x1*x3,
x2*x3^2 + 1/2*(nu^3 + 5*nu)*x3^2 - x2 + 1/2*(nu^3 + 5*nu),
x1*x4^2 + 1/2*(nu^3 + 5*nu)*x2*x3 - x3,
x1^2 - x2^2 + 1
] >;
X<x1, x2, x3, x4> := Curve(AA, I);
KX<x1, x2, x3, x4> := FunctionField(X);
s`SolvableDBBelyiCurve := X;
s`SolvableDBBelyiMap := KX!(-1/(x4^8 - 1));

/*
Graph Data
*/

s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,2,1-g0-path1", "4T2-2,2,2-g0-path3", "8T4-4,2,2-g0-path3", "16T10-4,4,2-g1-path11", "32S7-8,8,2-g5-path3" ];
s`SolvableDBParents := [ Strings() | "64S4-8,8,4-g17-path39", "64S24-8,8,4-g17-path86", "64S10-8,8,4-g17-path27", "64S14-8,8,4-g17-path39", "64S4-8,8,2-g9-path39", "64S12-8,8,2-g9-path5", "64S10-8,8,2-g9-path27" ];
