...
s`SolvableDBSanityCheckTiming := 1.010p15;
s`SolvableDBLocalSanityCheckTiming := 1.420p15;
...
x3*x4^6 + x3^3*x4^2 + x1*x4^4 + 2*x2*x3*x4^2 + x4^4 + x3^2 + x2 - 1,
x2*x3*x4^4 + x3^2*x4^2 - x1*x4^2 + 2*x2*x4^2 - x2*x3 + x4^2 + x3,
x1^2*x4^2 + x1*x2*x3 + x2^2*x3 - x2*x4^2,
x1*x2*x4^2 + x2^2*x3 + x1*x4^2 - x2*x4^2 + 2*x1*x3 + x2*x3 - x4^2,
x2^2*x4^2 + 2*x1*x3 + 2*x2*x3 - x4^2,
x1*x3*x4^2 + x1 + x2,
x2^3 - 2*x1^2 + x2,
x1*x3^2 - 1/2*x2^2 + 1/2,
x2*x3^2 - x3*x4^2 - x1 - 1
] >;
X<x1, x2, x3, x4> := Curve(AA, I);
KX<x1, x2, x3, x4> := FunctionField(X);
s`SolvableDBBelyiCurve := X;
s`SolvableDBBelyiMap := KX!(-1/x3^2*x4^4 - 2/x3^3*x4^2 - 1/x3^4);

/*
Graph Data
*/

s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,2,1-g0-path1", "4T1-4,4,1-g0-path1", "8T2-4,4,2-g1-path1", "16T5-8,8,4-g5-path3", "32S17-16,16,8-g13-path5" ];
s`SolvableDBParents := [ Strings() | "64S27-16,16,8-g25-path10", "64S44-16,16,8-g25-path74", "64S29-16,16,8-g25-path28" ];
s`SolvableDBChildren := [ Strings() | "16T5-8,8,4-g5-path3" ];
