...
s`SolvableDBName := "32S15-8,8,8-g11-path2";
s`SolvableDBFilename := "32S15-8,8,8-g11-path2.m";
...
s`SolvableDBSanityCheckTiming := 3.290p15;
...
x1^3*x2*x3^2 - nu^2*x1^4 - x2^2 - nu^2,
x1^5 + nu^2*x2*x3^2 + x1,
x2*x3^4 - nu^2*x1^2*x2 - 2*nu^2*x1*x3^2
] >;
X<x1, x2, x3> := Curve(AA, I);
KX<x1, x2, x3> := FunctionField(X);
s`SolvableDBBelyiCurve := X;
s`SolvableDBBelyiMap := KX!(-1/x1^4*x3^4 + 2*nu^2/x1^6*x3^2 + 1/x1^8);

/*
Graph Data
*/

s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T1-1,4,4-g0-path1", "8T2-2,4,4-g1-path1", "16T8-4,4,4-g3-path1", "32S15-8,8,8-g11-path2" ];
s`SolvableDBParents := [ Strings() | "64S15-8,8,8-g21-path252", "64S16-8,8,8-g21-path252", "64S22-8,8,8-g21-path12" ];
