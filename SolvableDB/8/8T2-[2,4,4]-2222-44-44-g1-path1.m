...
x0 - x1^2 - 1,
x1*x2^2 - 1,
-x1^2*x2 + x1^2*x3^2 + 1/4*(-F.1^3 + F.1)*x1^2 - x1*x2 + (-F.1^3 + F.1)*x1 + x3^2 + 1/4*(-F.1^3 + F.1)
] >;
Append(~s`SolvableDBCurveIdealList, I3);
X3<x0, x1, x2, x3> := Curve(AA3, ideal< PX3 | [
x0 - x1^2 - 1,
x1*x2 - x1*x3^2 + 1/4*(F.1^3 - F.1)*x1 + (-F.1^3 + F.1)*x2*x3^2 + 1/2*x2 + x3^2 + 1/4*(3*F.1^3 - 3*F.1),
x1*x3^4 + 1/2*(-F.1^3 + F.1)*x1*x3^2 + 1/8*x1 + (F.1^3 - F.1)*x2*x3^4 + 1/8*(-F.1^3 + F.1)*x2 - x3^4 + 1/2*(-3*F.1^3 + 3*F.1)*x3^2 - 1/8,
x2^2*x3^2 + 1/4*(-F.1^3 + F.1)*x2^2 + (-F.1^3 + F.1)*x2*x3^2 - 1/2*x2 + x3^2 + 1/4*(-F.1^3 + F.1)
] >);
