...
s`SolvableDBName := "16T5-8,8,4-g5-path6";
s`SolvableDBFilename := "16T5-8,8,4-g5-path6.m";
...
{ IntegerRing() | 12, 16 }
@};
s`SolvableDBBelyiMapTiming := 0.120000000000000p15;
s`SolvableDBSanityCheckTiming := 0.100000000000000p15;
...
x1*x2^2 - x1 - x2^3*x3^2 - x2*x3^2,
x1*x3^2 - 1/2*x2^2 + 1/2,
x2^4 - 2*x2^3*x3^4 - 2*x2^2 - 2*x2*x3^4 + 1
] >;
X<x1, x2, x3> := Curve(AA, I);
KX<x1, x2, x3> := FunctionField(X);
s`SolvableDBBelyiCurve := X;
s`SolvableDBBelyiMap := KX!(1/4*x2^3 - x3^4/2*x2^2 - 3/4*x2 + (-x3^4 + 2)/4);

/*
Graph Data
*/

s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,2,1-g0-path1", "4T2-2,2,2-g0-path3", "8T2-4,4,2-g1-path5", "16T5-8,8,4-g5-path6" ];
s`SolvableDBParents := [ Strings() | "32S16-16,16,8-g13-path14", "32S17-16,16,8-g13-path10", "32S16-16,16,8-g13-path15", "32S17-16,16,8-g13-path11", "32S12-8,8,4-g9-path14", "32S3-8,8,4-g9-path29", "32S5-8,8,4-g9-path18" ];
