...
s`SolvableDBName := "16T10-[2,4,4]-2,2,2,2,2,2,2,2-4,4,4,4-4,4,4,4-g1-path6";
s`SolvableDBFilename := "16T10-[2,4,4]-2,2,2,2,2,2,2,2-4,4,4,4-4,4,4,4-g1-path6.m";
...
[ 2, 13, 4, 15, 6, 9, 8, 11, 10, 5, 12, 7, 14, 1, 16, 3 ]
]
];
...
\[ 2, 13, 4, 15, 6, 9, 8, 11, 10, 5, 12, 7, 14, 1, 16, 3 ]:
 Order := 512 > |
[ 16, 7, 14, 5, 12, 3, 10, 1, 8, 15, 6, 13, 4, 11, 2, 9 ],
[ 15, 12, 13, 10, 11, 16, 9, 14, 7, 4, 5, 2, 3, 8, 1, 6 ],
[ 14, 1, 16, 3, 10, 5, 12, 7, 6, 9, 8, 11, 2, 13, 4, 15 ]
]
];
...
x0 - x1^2 - 1,
x1*x2 - x1 + nu2*x2 - x3^2,
x1*x3^2 + 1/2*x3^4 - 1/2,
x2*x3^2 - nu2*x2 + x3^2 + nu2
] >;
...
x1*x3^2 + 1/2*x3^4 - 1/2,
x2*x3^2 - nu2*x2 + x3^2 + nu2,
-2*x1*x3 + x1*x4^2 + 1/2*(nu2 + 1)*x1 - x3^3 + nu2*x3 - nu2 + 1
] >;
Append(~s`SolvableDBCurveIdealList, I4);
X4<x0, x1, x2, x3, x4> := Curve(AA4, ideal< PX4 | [
x0 - x1^2 - 1,
x1*x2 - x1 + nu2*x2 - x3^2,
x1*x3 - 1/2*x1*x4^2 + 1/4*(-nu2 - 1)*x1 + 1/2*x3^3 - 1/2*nu2*x3 + 1/2*(nu2 - 1),
x1*x4^4 + (nu2 + 1)*x1*x4^2 + 1/2*nu2*x1 + (-2*nu2 + 2)*x4^2,
x2*x3 - 1/2*x2*x4^2 + 1/4*(-3*nu2 - 3)*x2 + 1/2*(nu2 - 1)*x3*x4^2 + 1/2*x3 - 1/2*x4^2 + 1/4*(nu2 + 1),
x2*x4^4 + (3*nu2 + 3)*x2*x4^2 + 1/2*nu2*x2 + (-nu2 + 1)*x3*x4^4 + 1/2*(-nu2 - 1)*x3 + x4^4 + (-nu2 - 1)*x4^2 + 1/2*nu2,
...
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-[1,2,2]-11-2-2-g0-path1", "4T2-[2,2,2]-22-22-22-g0-path1", "8T4-[2,2,4]-2222-2222-44-g0-path1", "16T10-[2,4,4]-2,2,2,2,2,2,2,2-4,4,4,4-4,4,4,4-g1-path6" ];
