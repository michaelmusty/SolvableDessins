...
/* Belyi curve */
K := Rationals();
X<[x]> := HyperellipticCurve([Polynomial(K, [ 0, 0, 0, 0, 1 ]), Polynomial(K, [ 1, 0, 0, 0, 1 ])]);

/* Belyi map */
K := Rationals();
X<[x]> := HyperellipticCurve([Polynomial(K, [ 0, 0, 0, 0, 1 ]), Polynomial(K, [ 1, 0, 0, 0, 1 ])]);
KX<[x]> := FunctionField(X);
phi := KX!((-x[1]^12 - x[1]^8 + x[1]^4 + 1)/(4*x[1]^8)*x[2] + (x[1]^12 + 3*x[1]^8 + 3*x[1]^4 + 1)/(4*x[1]^8));
/* assign to object */
...
\[ 2, 1, 7, 6, 8, 4, 3, 5, 11, 16, 9, 14, 15, 12, 13, 10 ]:
 Order := 2 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T1-1,4,4-g0-path1", "8T2-2,4,4-g1-path1", "16T5-2,8,8-g3-path4" ];
