...
/* Belyi curve */
K := Rationals();
X<[x]> := EllipticCurve(Polynomial(K, [ 0, 1/8, 3/4, 1 ]), Polynomial(K, []));
/* Belyi map */
K := Rationals();
X<[x]> := EllipticCurve(Polynomial(K, [ 0, 1/8, 3/4, 1 ]), Polynomial(K, []));KX<[x]> := FunctionField(X);
phi := KX!(4*x[1]^2/(4*x[1]^2 + 4*x[1] + 1));
/* assign to object */
...
s`SolvableDBChild := "2T1-2,1,2-g0-path1";
