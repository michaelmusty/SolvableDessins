...
I<[x]> := ideal< P | [
x[3]^4*x[4]^4 + x[3]^4 + 4*x[3]^2*x[4]^2 - x[4]^4 - 1,
x[2]*x[3]^2*x[4]^4 + x[2]*x[4]^4 + x[2]*x[3]^2 + 2*x[1]*x[4]^2 + 2*x[2]*x[4]^2 + x[2],
x[2]^2*x[4]^4 - 1/2*x[3]^2*x[4]^4 + 1/2*x[4]^4 - 1/2*x[3]^2 - x[4]^2 + 1/2,
x[2]^3*x[4]^2 - x[1]*x[2]^2 + x[2]*x[4]^2 + x[1],
x[1]^2*x[2]^2 - x[1]^2 + x[2]^2 + 1,
x[2]^2*x[3]^2 + 2*x[2]^2*x[4]^2 - x[3]^2*x[4]^2 + x[2]^2 + x[4]^2 - 2,
x[1]^2*x[4]^2 - x[2]^2*x[4]^2 + x[1]*x[2] - x[4]^2,
x[1]*x[2]*x[4]^2 + 1,
...
{ IntegerRing() | 58, 63 }
@};
s`SolvableDBBelyiMapTiming := 4.360p15;
s`SolvableDBLocalSanityCheckTiming := 36.120p15;
...
\[ 22, 5, 59, 40, 6, 39, 4, 60, 12, 18, 24, 1, 11, 15, 29, 20, 50, 27, 41, 63, 46, 9, 64, 28, 45, 58, 51, 13, 53, 21, 23, 25, 26, 44, 37, 43, 57, 7, 2, 38, 8, 33, 55, 3, 54, 61, 35, 16, 31, 52, 10, 62, 14, 32, 56, 36, 47, 42, 34, 19, 30, 17, 48, 49 ]:
 Order := 4 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,1,2-g0-path1", "4T2-2,2,2-g0-path2", "8T4-2,4,2-g0-path2", "16T10-2,4,4-g1-path5", "32S6-4,4,4-g5-path3", "64S37-4,8,8-g17-path2" ];
s`SolvableDBParents := [ Strings() | "128S78-4,8,8-g33-path5", "128S78-4,8,8-g33-path6", "128S51-4,8,8-g33-path3", "128S57-4,8,8-g33-path3", "128S86-4,8,8-g33-path5", "128S86-4,8,8-g33-path6", "128S126-4,8,8-g33-path9" ];
