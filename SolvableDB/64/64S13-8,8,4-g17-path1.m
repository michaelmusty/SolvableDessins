...
I<[x]> := ideal< P | [
x[3]^2*x[4]^4 + (4*nu^3 - 4*nu^2)*x[2]^2*x[3]^2 + (2*nu^3 - 3*nu^2 + 2*nu)*x[4]^4 + 4*nu^2*x[2]*x[3]^2 + (-4*nu^3 + 6*nu^2 - 4*nu - 2)*x[1]*x[4]^2 + 2*x[3]^2 + (-4*nu^3 + 4*nu^2)*x[2] + 4*nu^3 + 2*nu^2 - 4*nu,
x[1]*x[2]^2*x[3]^2 + (-nu^3 - nu^2 - nu)*x[1]*x[2]*x[3]^2 + 1/2*(-nu^3 - 2*nu^2 - nu)*x[3]^2*x[4]^2 + (nu^3 + nu^2 - 1)*x[1]*x[3]^2 - x[1]*x[2] + 1/2*(-nu^3 + nu - 2)*x[4]^2 + (-nu + 1)*x[1],
x[2]^3*x[3]^2 + (-nu^3 - nu)*x[2]^2*x[3]^2 - x[2]*x[3]^2 - x[2]^2 + (-nu^3 - nu)*x[2] + 1,
x[1]*x[3]^2*x[4]^2 + (nu^3 - nu)*x[2]^2*x[3]^2 + (-nu^3 + 2*nu^2 + nu)*x[2]*x[3]^2 - x[1]*x[4]^2 + (-nu^2 + 1)*x[3]^2 + (-nu^3 + nu)*x[2] + nu^3 + nu^2 - nu - 1,
x[1]*x[4]^4 - 2*x[1]^2*x[4]^2 + 2*x[4]^2 - 2*x[1],
x[1]^2*x[3]^2 + (-nu^3 + nu)*x[2]*x[3]^2 - x[1]*x[4]^2 + x[1]^2 - nu^2*x[3]^2 + nu^3 - nu - 1,
x[1]^2*x[2] + x[1]*x[4]^2 - x[1]^2 + nu^2*x[2] + 1,
x[2]*x[4]^2 + (nu^2 - 1)*x[1]*x[2] + nu^2*x[4]^2 + (-nu^2 + 1)*x[1]
] >;
...
s`SolvableDBName := "64S13-8,8,4-g17-path1";
s`SolvableDBFilename := "64S13-8,8,4-g17-path1.m";
...
{ IntegerRing() | 46, 53 }
@};
s`SolvableDBBelyiMapTiming := 5.050p15;
s`SolvableDBLocalSanityCheckTiming := 9.300p15;
...
\[ 6, 1, 27, 28, 24, 29, 4, 30, 2, 3, 5, 7, 8, 37, 14, 58, 13, 34, 61, 36, 39, 31, 52, 9, 21, 42, 35, 44, 11, 50, 19, 20, 22, 23, 41, 53, 57, 10, 12, 15, 16, 17, 18, 25, 26, 32, 33, 64, 48, 45, 62, 51, 60, 63, 59, 43, 49, 38, 47, 54, 55, 56, 46, 40 ]:
 Order := 8 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T2-2,2,2-g0-path1", "8T4-4,2,2-g0-path1", "16T10-4,4,2-g1-path5", "32S10-4,8,4-g7-path6", "64S13-8,8,4-g17-path1" ];
s`SolvableDBParents := [ Strings() | "128S96-16,8,4-g37-path1", "128S95-16,8,4-g37-path1", "128S97-16,8,4-g37-path1", "128S95-16,8,4-g37-path2", "128S3-8,8,4-g33-path15", "128S28-8,8,4-g33-path5", "128S3-8,8,4-g33-path16" ];
s`SolvableDBChild := "32S10-4,8,4-g7-path6";
