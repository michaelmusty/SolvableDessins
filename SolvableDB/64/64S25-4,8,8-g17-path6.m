...
x[1]^2*x[4]^2 - x[1]^2*x[3] - 2*x[1]*x[3] + x[4]^2 + x[3],
x[2]^2*x[4]^2 - 2*x[1]^2*x[3] - x[2]^2*x[3] + 2*x[1]*x[4]^2,
x[3]^2*x[4]^2 - x[1]*x[4]^2 + x[1]*x[3] + x[3],
x[1]^3 - x[2]^2 - x[1]
] >;
...
s`SolvableDBName := "64S25-4,8,8-g17-path6";
s`SolvableDBFilename := "64S25-4,8,8-g17-path6.m";
...
\[ 22, 5, 54, 40, 6, 39, 4, 62, 12, 18, 24, 1, 11, 15, 29, 59, 50, 27, 41, 16, 48, 9, 35, 28, 46, 45, 51, 13, 56, 21, 23, 25, 26, 44, 61, 43, 52, 7, 2, 38, 8, 33, 57, 3, 42, 60, 37, 58, 47, 53, 10, 49, 55, 34, 17, 14, 63, 30, 64, 32, 31, 19, 36, 20 ]:
 Order := 4 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,1,2-g0-path1", "4T2-2,2,2-g0-path2", "8T4-2,4,2-g0-path2", "16T10-4,4,2-g1-path4", "32S2-4,4,4-g5-path7", "64S25-4,8,8-g17-path6" ];
s`SolvableDBParents := [ Strings() | "128S33-8,8,8-g41-path49", "128S39-8,8,8-g41-path10", "128S34-8,8,8-g41-path31", "128S40-8,8,8-g41-path1", "128S15-8,8,8-g41-path55", "128S23-8,8,8-g41-path25", "128S13-8,8,8-g41-path19", "128S19-8,8,8-g41-path19", "128S18-4,8,8-g33-path15", "128S14-4,8,8-g33-path19", "128S20-4,8,8-g33-path9", "128S35-4,8,8-g33-path19", "128S37-4,8,8-g33-path13", "128S32-4,8,8-g33-path13", "128S38-4,8,8-g33-path4" ];
