...
x[1]^2*x[4]^2 + (nu^3 - nu)*x[2]*x[4]^2 + 1/2*(-2*nu^3 - nu^2 + 1)*x[1]^2 - nu^2*x[4]^2 + (-nu^3 - nu^2 + 1)*x[2] + 1/2*(-nu^2 - 2*nu - 1),
x[2]^2*x[4]^2 + (nu^3 + nu)*x[2]*x[4]^2 + 1/2*(2*nu^3 + nu^2 - 1)*x[2]^2 - x[4]^2 + (nu^2 + nu + 1)*x[2] + 1/2*(-2*nu^3 - nu^2 + 1),
x[3]^2 - x[1]
] >;
...
s`SolvableDBLocalSanityCheckTiming := 0.940p15;
...
\[ 6, 1, 22, 23, 21, 24, 4, 25, 2, 3, 5, 7, 8, 20, 51, 50, 52, 49, 10, 35, 45, 53, 17, 44, 43, 19, 16, 27, 36, 9, 11, 12, 13, 14, 15, 18, 26, 28, 29, 41, 56, 37, 62, 57, 63, 48, 34, 60, 40, 46, 55, 59, 42, 33, 47, 39, 30, 31, 32, 38, 54, 61, 64, 58 ]:
 Order := 8 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T2-2,2,2-g0-path1", "8T4-4,2,2-g0-path1", "16T10-4,4,2-g1-path5", "32S11-8,4,2-g3-path3", "64S9-8,4,4-g13-path14" ];
s`SolvableDBParents := [ Strings() | "128S3-8,8,4-g33-path80", "128S86-8,8,4-g33-path16", "128S4-8,8,4-g33-path38", "128S85-8,8,4-g33-path16", "128S84-8,4,4-g25-path11", "128S26-8,4,4-g25-path39", "128S83-8,4,4-g25-path11" ];
