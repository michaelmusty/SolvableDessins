...
I<[x]> := ideal< P | [
x[3]^2*x[4]^4 + 1/8*x[1]*x[4]^4 + 1/2*x[1]*x[2]*x[3]^2 - 1/4*x[2]^2*x[3]^2 + 4*x[3]^2*x[4]^2 + 2*x[1]*x[3]^2 - x[2]*x[3]^2 + 1/2*x[1]*x[4]^2 - 1/4*x[1]^2 + 1/8*x[1]*x[2] + 1/16*x[2]^2 + 4*x[3]^2 + x[4]^2 - x[1] - 5/4*x[2] + 1,
x[1]*x[2]^2*x[3]^2 + 8*x[1]*x[2]*x[3]^2 - 16*x[3]^2*x[4]^2 - x[1]^2*x[2] - 1/4*x[1]*x[2]^2 + 16*x[1]*x[3]^2 + 24*x[2]*x[3]^2 - 2*x[1]*x[4]^2 + 6*x[1]^2 + 2*x[1]*x[2] - x[2]^2 + 2*x[2],
x[2]^3*x[3]^2 + 8*x[2]^2*x[3]^2 - 96*x[3]^2*x[4]^2 - x[1]^2*x[2] - x[1]*x[2]^2 - 1/4*x[2]^3 - 96*x[1]*x[3]^2 + 64*x[2]*x[3]^2 - 12*x[1]*x[4]^2 + 8*x[1]^2 + 6*x[1]*x[2] + 2*x[2]^2 - 64*x[3]^2 - 8*x[1] + 8*x[2] - 16,
x[1]*x[3]^2*x[4]^2 - 1/2*x[1]*x[2]*x[3]^2 + 1/4*x[2]^2*x[3]^2 - 4*x[3]^2*x[4]^2 - 4*x[1]*x[3]^2 + 2*x[2]*x[3]^2 - 1/2*x[1]*x[4]^2 + 1/4*x[1]^2 - 1/8*x[1]*x[2] - 1/16*x[2]^2 - 4*x[3]^2 - 1/2*x[4]^2 + 1/2*x[1] + x[2] - 1,
x[1]^2*x[3]^2 + 4*x[3]^2*x[4]^2 + 4*x[1]*x[3]^2 - 2*x[2]*x[3]^2 + 1/2*x[1]*x[4]^2 - 1/4*x[1]^2 + 4*x[3]^2 - 1/2*x[2] + 1,
...
s`SolvableDBName := "64S13-8,4,8-g17-path5";
s`SolvableDBFilename := "64S13-8,4,8-g17-path5.m";
...
{ IntegerRing() | 44, 50 }
@};
s`SolvableDBBelyiMapTiming := 6.910p15;
...
\[ 6, 1, 27, 28, 24, 29, 4, 30, 2, 3, 5, 7, 8, 23, 44, 58, 13, 14, 17, 35, 59, 31, 9, 60, 21, 61, 34, 10, 18, 33, 19, 20, 22, 12, 39, 43, 56, 11, 15, 16, 25, 26, 32, 36, 37, 57, 49, 62, 50, 55, 42, 46, 45, 51, 53, 47, 38, 64, 40, 52, 63, 54, 48, 41 ]:
 Order := 8 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T2-2,2,2-g0-path1", "8T4-2,2,4-g0-path1", "16T10-4,2,4-g1-path4", "32S10-8,4,4-g7-path3", "64S13-8,4,8-g17-path5" ];
s`SolvableDBParents := [ Strings() | "128S96-8,4,16-g37-path2", "128S95-8,4,16-g37-path3", "128S95-8,4,16-g37-path4", "128S97-8,4,16-g37-path2", "128S3-8,4,8-g33-path19", "128S3-8,4,8-g33-path20", "128S28-8,4,8-g33-path7" ];
s`SolvableDBChild := "32S10-8,4,4-g7-path3";
