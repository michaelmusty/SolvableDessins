...
I<[x]> := ideal< P | [
x[2]^3*x[4]^4 + x[2]*x[3]^2*x[4]^4 + x[1]*x[4]^4 + 2*x[1]*x[2]^2 + x[2]^3 + 3*x[3]*x[4]^2 - 2*x[1] - x[2],
x[1]^2*x[4]^4 + x[2]^2*x[4]^4 + x[3]^2*x[4]^4 - 2*x[2]*x[3]*x[4]^2 + x[2]^2,
x[2]^3*x[3]^2 - 2*x[1]*x[2]^2 + x[2]^3 - x[1]*x[3]^2 + x[1] + 2*x[2],
x[1]^3*x[4]^2 - x[2]*x[3]^2*x[4]^2 - x[1]^2*x[3] - 2*x[1]*x[2]*x[3] + x[2]*x[4]^2 - 2*x[3],
x[2]^2*x[3]*x[4]^2 + x[1]*x[2]^2 + x[3]*x[4]^2 - x[1],
x[3]^3*x[4]^2 - x[2]*x[3]^2 - x[3]*x[4]^2 - x[2],
x[1]^2*x[2]^2 - x[1]^2 - x[2]^2 - 1,
x[1]*x[2]^2*x[3] - x[1]^2*x[4]^2 - x[2]^2*x[4]^2 - x[3]^2*x[4]^2 + 2*x[2]*x[3],
x[1]^2*x[3]^2 + x[2]^2*x[3]^2 + x[1]^2 - 2*x[1]*x[2] + x[2]^2 + 2,
x[1]*x[2]*x[3]^2 + x[1]*x[2] + x[3]^2 - 1,
x[1]*x[2]*x[4]^2 + x[3]^2*x[4]^2 - x[2]*x[3] - x[4]^2,
x[1]*x[3]*x[4]^2 + 1
] >;
...
phi := KX!((-8*x[3]^10 - 8*x[3]^6)/(x[3]^12 + 2*x[3]^10 - x[3]^8 - 4*x[3]^6 - x[3]^4 + 2*x[3]^2 + 1)*x[4]^4 + (x[3]^12 - 2*x[3]^10 - x[3]^8 - 12*x[3]^6 - x[3]^4 - 2*x[3]^2 + 1)/(x[3]^12 - 2*x[3]^10 - x[3]^8 + 4*x[3]^6 - x[3]^4 - 2*x[3]^2 + 1));
/* assign to object */
...
s`SolvableDBName := "64S17-8,8,4-g17-path11";
s`SolvableDBFilename := "64S17-8,8,4-g17-path11.m";
...
{ IntegerRing() | 52, 59 }
@};
s`SolvableDBBelyiMapTiming := 0.650p15;
s`SolvableDBLocalSanityCheckTiming := 1.280p15;
...
\[ 6, 1, 27, 28, 22, 29, 24, 30, 2, 3, 4, 5, 7, 8, 44, 26, 59, 55, 14, 34, 48, 9, 53, 62, 31, 45, 35, 13, 12, 56, 50, 51, 57, 61, 43, 60, 42, 10, 11, 15, 16, 17, 18, 19, 20, 21, 23, 25, 32, 33, 36, 37, 49, 64, 38, 40, 46, 52, 54, 47, 63, 39, 41, 58 ]:
 Order := 8 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T2-2,2,2-g0-path1", "8T4-4,2,2-g0-path1", "16T10-4,2,4-g1-path5", "32S2-4,4,4-g5-path5", "64S17-8,8,4-g17-path11" ];
s`SolvableDBParents := [ Strings() | "128S5-8,8,8-g41-path15", "128S9-8,8,8-g41-path51", "128S10-8,8,8-g41-path29", "128S15-8,8,8-g41-path29", "128S10-8,8,8-g41-path30", "128S15-8,8,8-g41-path30", "128S13-8,8,8-g41-path15", "128S7-8,8,8-g41-path15", "128S106-16,16,4-g41-path3", "128S106-16,16,8-g49-path3", "128S108-16,16,4-g41-path3", "128S108-16,16,8-g49-path3", "128S111-16,16,4-g41-path5", "128S111-16,16,8-g49-path5", "128S110-16,16,4-g41-path5", "128S110-16,16,8-g49-path5", "128S111-16,16,4-g41-path6", "128S111-16,16,8-g49-path6", "128S110-16,16,4-g41-path6", "128S110-16,16,8-g49-path6", "128S109-16,16,4-g41-path3", "128S109-16,16,8-g49-path3", "128S107-16,16,4-g41-path3", "128S107-16,16,8-g49-path3", "128S8-8,8,4-g33-path3", "128S11-8,8,4-g33-path5", "128S14-8,8,4-g33-path5", "128S11-8,8,4-g33-path6", "128S14-8,8,4-g33-path6", "128S12-8,8,4-g33-path3", "128S6-8,8,4-g33-path3" ];
