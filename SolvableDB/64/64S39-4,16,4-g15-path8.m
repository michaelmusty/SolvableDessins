...
I<[x]> := ideal< P | [
x[1]^3*x[4]^2 + (2*nu^3 - 4*nu^2 + 2*nu)*x[1]^3*x[3] + (4*nu^3 - 4*nu + 4)*x[1]^2*x[2]*x[3] + nu^2*x[1]^2*x[4]^2 + (-nu^3 - nu)*x[1]*x[2]*x[4]^2 + 2*x[2]^2*x[4]^2 + (-2*nu^3 + 8*nu^2 - 6*nu + 4)*x[1]^2*x[3] + (-8*nu^3 + 4*nu^2 - 4)*x[1]*x[2]*x[3] - nu^2*x[1]*x[4]^2 + (-nu^3 - nu)*x[2]*x[4]^2 + (-2*nu^3 - 4*nu^2 + 6*nu - 8)*x[1]*x[3] + (4*nu^3 - 4*nu^2 + 4*nu)*x[2]*x[3] + x[4]^2 + (2*nu^3 - 2*nu + 4)*x[3],
x[1]^2*x[2]*x[4]^2 + (2*nu^3 - 4*nu^2 + 2*nu)*x[1]^2*x[2]*x[3] - nu*x[1]^2*x[4]^2 + 2*x[1]*x[2]*x[4]^2 + (nu^3 + nu)*x[2]^2*x[4]^2 + (4*nu^3 - 2*nu^2 + 2)*x[1]^2*x[3] + (4*nu^2 - 4*nu + 4)*x[1]*x[2]*x[3] + x[2]*x[4]^2 + (-4*nu^3 + 4*nu^2 - 4*nu)*x[1]*x[3] + (-2*nu^3 + 2*nu - 4)*x[2]*x[3] + nu^3*x[4]^2 + (-2*nu^2 + 4*nu - 2)*x[3],
x[1]*x[2]^2*x[4]^2 + x[1]^2*x[4]^2 + nu^3*x[1]*x[2]*x[4]^2 - nu^2*x[2]^2*x[4]^2 + (2*nu^3 - 4*nu^2 + 2*nu)*x[1]^2*x[3] + (4*nu^3 - 2*nu^2 + 2)*x[1]*x[2]*x[3] + nu^3*x[2]*x[4]^2 + (-2*nu^3 + 4*nu^2 - 2*nu)*x[1]*x[3] + (-4*nu^3 + 2*nu^2 - 2)*x[2]*x[3] - nu^2*x[4]^2,
x[2]^3*x[4]^2 + (-4*nu^3 + 2*nu^2 - 2)*x[1]*x[2]^2*x[3] + nu^3*x[1]^2*x[4]^2 - 2*nu^2*x[1]*x[2]*x[4]^2 + (-nu^3 + nu)*x[2]^2*x[4]^2 + (-2*nu^2 + 4*nu - 2)*x[1]^2*x[3] + (-4*nu^2 + 4*nu - 4)*x[1]*x[2]*x[3] + (4*nu^3 - 4*nu^2 + 4*nu)*x[2]^2*x[3] - nu^2*x[2]*x[4]^2 + (4*nu^3 + 2*nu^2 - 8*nu + 6)*x[1]*x[3] + (4*nu^2 - 4*nu + 4)*x[2]*x[3] + nu*x[4]^2 + (-2*nu^2 + 4*nu - 2)*x[3],
x[1]^2*x[3]*x[4]^2 - nu^2*x[2]^2*x[3]*x[4]^2 + (2*nu^3 - 4*nu^2 + 2*nu)*x[1]^2*x[2] + (2*nu^2 - 4*nu + 2)*x[1]*x[2]^2 + (-4*nu^3 + 8*nu^2 - 4*nu)*x[1]*x[2] + (-4*nu^3 + 4*nu - 4)*x[2]^2 + (4*nu^3 - 2*nu^2 + 2)*x[1] + (2*nu^3 - 4*nu^2 + 2*nu)*x[2] - 4*nu^3 + 4*nu - 4,
x[1]*x[2]*x[3]*x[4]^2 + nu*x[2]^2*x[3]*x[4]^2 - nu^3*x[1]*x[3]*x[4]^2 + x[2]*x[3]*x[4]^2 + (2*nu^3 - 2*nu + 4)*x[1]*x[2]^2 + (2*nu^2 - 4*nu + 2)*x[1]*x[2] + (-4*nu^3 + 4*nu^2 - 4)*x[2]^2 + (-4*nu^2 + 4*nu - 4)*x[1] + (-2*nu^2 + 4*nu - 2)*x[2] - 2*nu^3 + 4*nu^2 - 2*nu,
...
{ IntegerRing() | 63, 64 }
@};
s`SolvableDBBelyiMapTiming := 1.740p15;
s`SolvableDBLocalSanityCheckTiming := 16.070p15;
...
\[ 20, 5, 25, 24, 6, 30, 4, 23, 12, 16, 8, 1, 11, 44, 34, 22, 7, 27, 26, 9, 32, 28, 13, 17, 19, 3, 21, 10, 33, 2, 37, 18, 15, 29, 60, 49, 43, 50, 45, 48, 31, 46, 41, 39, 14, 36, 53, 38, 42, 40, 58, 51, 59, 62, 61, 63, 47, 64, 57, 55, 35, 56, 54, 52 ]:
 Order := 4 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T2-2,2,2-g0-path1", "8T4-4,2,2-g0-path1", "16T10-4,4,2-g1-path5", "32S9-4,8,2-g3-path3", "64S39-4,16,4-g15-path8" ];
s`SolvableDBParents := [ Strings() | "128S64-8,16,4-g37-path1", "128S94-8,16,4-g37-path4", "128S64-8,16,4-g37-path2", "128S97-8,16,4-g37-path4", "128S80-4,16,4-g29-path7", "128S112-4,16,4-g29-path16", "128S80-4,16,4-g29-path8" ];
