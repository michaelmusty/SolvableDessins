...
I<[x]> := ideal< P | [
x[3]^2*x[4]^4 + x[3]^2*x[4]^2 - 1/16*x[4]^4 - 1/4*x[2]*x[3]^2 + 1/32*x[2]*x[4]^2 + x[3]^2 - 1/16*x[4]^2 - 1/16*x[1] - 1/64*x[2] - 1/16,
x[2]*x[3]^2*x[4]^2 + 2*x[1]*x[3]^2 - 1/16*x[2]*x[4]^2 + 1/8*x[1],
x[1]^2*x[3]^2 + 8*x[3]^2*x[4]^2 + 4*x[1]*x[3]^2 + 2*x[2]*x[3]^2 - 1/16*x[1]^2 + 4*x[3]^2 - 1/2*x[4]^2 - 1/4*x[1] + 1/8*x[2] - 1/4,
x[1]*x[2]*x[3]^2 - 16*x[3]^2*x[4]^2 - 12*x[1]*x[3]^2 - 2*x[2]*x[3]^2 + 1/4*x[1]^2 - 1/16*x[1]*x[2] - 8*x[3]^2 + x[4]^2 + 1/4*x[1] - 3/8*x[2] + 1/2,
x[2]^2*x[3]^2 + 16*x[3]^2*x[4]^2 + 16*x[1]*x[3]^2 - 4*x[2]*x[3]^2 - 1/2*x[1]^2 + 1/4*x[1]*x[2] - 1/16*x[2]^2 + 16*x[3]^2 - x[4]^2 + 1/4*x[2] - 1,
...
s`SolvableDBLocalSanityCheckTiming := 6.080p15;
...
\[ 6, 1, 27, 28, 24, 29, 4, 30, 2, 3, 5, 7, 8, 23, 58, 61, 56, 14, 17, 64, 34, 26, 52, 9, 21, 62, 60, 42, 11, 33, 19, 20, 22, 12, 39, 44, 10, 13, 15, 16, 18, 25, 31, 32, 35, 36, 57, 59, 45, 43, 47, 51, 49, 50, 53, 63, 41, 55, 46, 40, 37, 38, 54, 48 ]:
 Order := 8 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T2-2,2,2-g0-path1", "8T4-2,2,4-g0-path1", "16T10-4,2,4-g1-path4", "32S6-4,4,4-g5-path6", "64S9-8,4,4-g13-path20" ];
s`SolvableDBParents := [ Strings() | "128S3-8,4,8-g33-path80", "128S86-8,4,8-g33-path16", "128S4-8,4,8-g33-path38", "128S85-8,4,8-g33-path16", "128S84-8,4,4-g25-path14", "128S26-8,4,4-g25-path42", "128S83-8,4,4-g25-path14" ];
