...
I<[x]> := ideal< P | [
x[2]^2*x[4]^4 + x[1]^3*x[2] + 4*x[4]^4,
x[1]^4*x[2] + x[2]^2 + 4,
x[1]^3*x[2]*x[3] + x[2]^2*x[4]^2 + 4*x[4]^2,
x[1]*x[4]^2 - x[3],
x[3]*x[4]^2 - 1,
x[3]^2 - x[1]
] >;
...
phi := KX!((x[3]^16 - 8*x[3]^8 + 16)/(x[3]^16 + 8*x[3]^8 + 16));
/* assign to object */
...
\[ 19, 5, 27, 13, 6, 20, 4, 11, 12, 17, 21, 1, 14, 7, 45, 32, 24, 28, 9, 2, 25, 26, 34, 29, 8, 3, 22, 23, 10, 33, 37, 30, 16, 18, 61, 49, 43, 48, 44, 50, 31, 46, 41, 15, 39, 36, 53, 40, 42, 38, 52, 63, 59, 62, 60, 64, 47, 51, 57, 35, 55, 56, 58, 54 ]:
 Order := 4 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,1,2-g0-path1", "4T1-4,1,4-g0-path1", "8T2-4,2,4-g1-path1", "16T8-4,4,4-g3-path5", "32S14-4,8,4-g7-path5", "64S48-4,16,4-g15-path1" ];
s`SolvableDBParents := [ Strings() | "128S158-8,32,4-g39-path3", "128S158-4,32,8-g39-path3", "128S104-8,16,8-g45-path9", "128S158-8,32,4-g39-path4", "128S158-4,32,8-g39-path4", "128S104-8,16,8-g45-path10", "128S112-4,16,4-g29-path23" ];
