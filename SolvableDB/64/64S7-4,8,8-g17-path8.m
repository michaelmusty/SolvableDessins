...
I<[x]> := ideal< P | [
x[3]^2*x[4]^4 - x[1]*x[4]^4 + 8*x[1]*x[2]*x[3]^2 - 4*x[2]^2*x[3]^2 + 4*x[1]*x[2]^2 - 4*x[4]^2,
x[1]*x[2]^2*x[3]^2 + 1/2*x[3]^2*x[4]^2 - x[1]*x[3]^2 + x[2]*x[3]^2 - 1/2*x[1]*x[4]^2 + x[1]*x[2] + x[2]^2 - 1,
x[2]^3*x[3]^2 - x[1]*x[2]^3 + 1/2*x[3]^2*x[4]^2 - 2*x[1]*x[3]^2 + 2*x[2]*x[3]^2 - 1/2*x[1]*x[4]^2 + 2*x[1]*x[2] + 2*x[2]^2,
x[1]*x[3]^2*x[4]^2 + 2*x[1]*x[2]*x[3]^2 - 2*x[2]^2*x[3]^2 + 2*x[1]*x[2]^2 - 2*x[3]^2 - x[4]^2 - 2*x[1] - 2*x[2],
x[1]^2*x[2]^2 - x[1]^2 + x[2]^2 + 1,
x[1]^2*x[3]^2 - x[1]*x[2]*x[3]^2 - x[1] - x[2],
x[1]^2*x[4]^2 - 2*x[1]^2*x[2] - x[4]^2 - 2*x[2],
x[2]*x[4]^2 - 2
] >;
...
s`SolvableDBName := "64S7-4,8,8-g17-path8";
s`SolvableDBFilename := "64S7-4,8,8-g17-path8.m";
...
s`SolvableDBLocalSanityCheckTiming := 1.910p15;
...
\[ 22, 5, 55, 41, 6, 40, 4, 46, 12, 18, 24, 1, 11, 15, 29, 20, 63, 27, 14, 51, 32, 9, 57, 28, 30, 47, 34, 13, 19, 21, 23, 25, 26, 10, 53, 64, 35, 45, 7, 2, 39, 43, 8, 60, 3, 42, 59, 54, 37, 52, 62, 48, 49, 50, 38, 58, 61, 44, 33, 56, 31, 16, 36, 17 ]:
 Order := 4 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,1,2-g0-path1", "4T2-2,2,2-g0-path2", "8T4-2,4,2-g0-path2", "16T10-2,4,4-g1-path5", "32S10-4,8,4-g7-path3", "64S7-4,8,8-g17-path8" ];
s`SolvableDBParents := [ Strings() | "128S70-4,16,16-g41-path15", "128S70-4,16,16-g41-path16", "128S69-4,16,16-g41-path23", "128S69-4,16,16-g41-path24", "128S11-4,8,8-g33-path49", "128S3-4,8,8-g33-path55", "128S4-4,8,8-g33-path25" ];
