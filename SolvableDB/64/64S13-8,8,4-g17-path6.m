...
I<[x]> := ideal< P | [
x[2]^2*x[4]^4 + 1/2*(-nu^6 + 4*nu^2)*x[1]^4 - 1/4*nu^6*x[4]^4 - 2*nu^4*x[1]^3 + 1/2*(nu^6 - 4*nu^2)*x[3]*x[4]^2 + 1/2*(nu^6 - 4*nu^2)*x[1]^2 + 1/2*nu^4*x[2]^2 - 2*nu^2,
x[1]^5 - x[2]^2 - x[1],
x[1]^4*x[3] - 1/8*nu^6*x[1]^3*x[3] - 1/16*nu^6*x[2]^2*x[4]^2 - 1/4*nu^4*x[1]^2*x[3] + 1/16*(nu^6 + 4*nu^2)*x[2]^2*x[3] - 1/2*nu^2*x[1]*x[3] - 1/4*nu^4*x[4]^2 + 1/4*(nu^4 - 4)*x[3],
x[2]^2*x[3]*x[4]^2 + nu^2*x[1]^4 - 1/2*nu^4*x[1]^3 - nu^2*x[3]*x[4]^2 + 1/4*nu^6*x[1]^2 + 1/4*(nu^4 - 4)*x[2]^2 + 2*x[1] + 1/4*(-nu^6 - 4*nu^2),
x[3]*x[4]^4 - 2*x[3]^2*x[4]^2 + 1/2*nu^4*x[4]^2 - 1/2*nu^4*x[3],
x[2]^2*x[3]^2 - 1/2*nu^4*x[1]^3 - nu^2*x[3]*x[4]^2 - x[2]^2 + nu^2*x[3]^2 + 2*x[1] - 1/4*nu^6,
x[1]*x[3]^2 + 1/2*nu^2*x[3]*x[4]^2 - 1/2*nu^2*x[3]^2 - x[1] + 1/8*nu^6,
x[1]*x[4]^2 + 1/4*(nu^4 - 4)*x[1]*x[3] + 1/8*nu^6*x[4]^2 + 1/8*(-nu^6 + 4*nu^2)*x[3]
] >;
...
s`SolvableDBName := "64S13-8,8,4-g17-path6";
s`SolvableDBFilename := "64S13-8,8,4-g17-path6.m";
...
{ IntegerRing() | 45, 50 }
@};
s`SolvableDBBelyiMapTiming := 17.260p15;
s`SolvableDBLocalSanityCheckTiming := 10.070p15;
...
\[ 32, 29, 33, 13, 61, 20, 17, 7, 6, 35, 59, 28, 12, 49, 42, 51, 30, 40, 27, 2, 44, 10, 45, 55, 52, 25, 22, 8, 36, 4, 3, 9, 19, 15, 31, 1, 34, 24, 48, 37, 62, 23, 58, 43, 18, 56, 16, 64, 53, 39, 54, 60, 57, 46, 11, 41, 50, 26, 63, 21, 38, 47, 5, 14 ]:
 Order := 8 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T2-2,2,2-g0-path1", "8T4-4,2,2-g0-path1", "16T12-8,2,4-g2-path1", "32S10-8,4,4-g7-path7", "64S13-8,8,4-g17-path6" ];
s`SolvableDBParents := [ Strings() | "128S96-8,16,4-g37-path2", "128S95-8,16,4-g37-path3", "128S95-8,16,4-g37-path4", "128S97-8,16,4-g37-path2", "128S3-8,8,4-g33-path21", "128S3-8,8,4-g33-path22", "128S28-8,8,4-g33-path8" ];
