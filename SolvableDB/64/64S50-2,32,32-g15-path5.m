...
I<[x]> := ideal< P | [
x[1] - x[3]^2,
x[2]^2*x[3]^16 + (nu^13 - nu^9 + nu^7 - nu^5)*x[2]^2*x[3]^14 + (nu^14 - nu^12 + nu^6 - nu^4 + nu^2 + 1)*x[2]^2*x[3]^12 + (nu^13 - nu^11 + nu^9 - nu^5)*x[2]^2*x[3]^10 + (-nu^12 + nu^2)*x[2]^2*x[3]^8 + (nu^9 - nu^5 + nu^3 - nu)*x[2]^2*x[3]^6 + (-nu^14 - nu^12 + nu^10 - nu^8 + nu^2 - 1)*x[2]^2*x[3]^4 + (nu^9 - nu^7 + nu^5 - nu)*x[2]^2*x[3]^2 - nu^14*x[2]^2 - x[3]^16 + (nu^13 - nu^9 + nu^7 - nu^5)*x[3]^14 + (-2*nu^14 + nu^12 - nu^6 + nu^4 - 1)*x[3]^12 + (-nu^15 + nu^13 - nu^11 + 2*nu^7 - 2*nu^5 + nu^3)*x[3]^10 + (-2*nu^14 + nu^12 - nu^10 + nu^8 - nu^6 + 2*nu^4 - nu^2 - 1)*x[3]^8 + (-nu^15 + 2*nu^13 - 2*nu^11 + nu^7 - nu^5 + nu^3)*x[3]^6 + (-nu^14 + nu^12 - nu^6 + 2*nu^4 - nu^2)*x[3]^4 + (nu^13 - nu^11 + nu^9 - nu^5)*x[3]^2 - nu^2
] >;
...
s`SolvableDBName := "64S50-2,32,32-g15-path5";
s`SolvableDBFilename := "64S50-2,32,32-g15-path5.m";
...
{ IntegerRing() | 59, 63 }
@};
s`SolvableDBBelyiMapTiming := 23.7800000000000p15;
s`SolvableDBLocalSanityCheckTiming := 1.76000000000000p15;
...
\[ 2, 1, 7, 6, 8, 4, 3, 5, 11, 16, 9, 14, 15, 12, 13, 10, 19, 20, 17, 18, 23, 24, 21, 22, 27, 28, 25, 26, 31, 32, 29, 30, 35, 36, 33, 34, 39, 40, 37, 38, 43, 44, 41, 42, 47, 48, 45, 46, 51, 52, 49, 50, 55, 56, 53, 54, 59, 60, 57, 58, 63, 64, 61, 62 ]:
 Order := 2 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T1-1,4,4-g0-path1", "8T1-1,8,8-g0-path1", "16T1-1,16,16-g0-path1", "32S16-2,16,16-g7-path1", "64S50-2,32,32-g15-path5" ];
s`SolvableDBParents := [ Strings() | "128S128-4,32,32-g45-path5", "128S153-4,32,32-g45-path8", "128S159-2,64,64-g31-path4", "128S159-4,64,64-g47-path4", "128S160-2,64,64-g31-path2", "128S160-4,64,64-g47-path2", "128S131-2,32,32-g29-path5" ];
