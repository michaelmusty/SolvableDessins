...
I<[x]> := ideal< P | [
x[1]^9*x[2]^2 + (-nu^11 - nu^9 - nu^7 + nu^5 - nu^3 - nu)*x[1]^8*x[2]^2 + (nu^14 + nu^12 + 2*nu^10 - nu^6 - nu^2)*x[1]^7*x[2]^2 - x[1]^8 + (nu^9 + nu^7 + 2*nu^5)*x[1]^6*x[2]^2 + (-nu^11 - nu^9 - nu^7 + nu^5 - nu^3 - nu)*x[1]^7 + (-nu^14 + nu^10 - nu^8 + nu^4 + nu^2 + 1)*x[1]^5*x[2]^2 + (-nu^12 - 3*nu^10 + nu^6 + nu^2)*x[1]^6 + (-2*nu^13 - nu^11 - nu^9)*x[1]^4*x[2]^2 + (2*nu^15 - nu^13 - nu^11 + 2*nu^5 + 2*nu^3)*x[1]^5 + (nu^12 - 2*nu^8 - nu^6 - nu^4 - 1)*x[1]^3*x[2]^2 + (nu^14 + nu^12 - 2*nu^10 - nu^8 + nu^6 + 2*nu^4 - nu^2 - 1)*x[1]^4 + (nu^15 - nu^13 + nu^11 + nu^9 + nu^7 - nu)*x[1]^2*x[2]^2 + (2*nu^15 - 2*nu^11 - 2*nu^9 + nu^3 + nu)*x[1]^3 + nu^2*x[1]*x[2]^2 + (-nu^12 - nu^8 + 3*nu^4 + nu^2)*x[1]^2 + (nu^13 + nu^11 - nu^9 + nu^7 + nu^5 + nu^3)*x[1] + nu^14
] >;
...
s`SolvableDBName := "64S53-4,32,2-g8-path1";
s`SolvableDBFilename := "64S53-4,32,2-g8-path1.m";
...
{ IntegerRing() | 60, 62 }
@};
s`SolvableDBBelyiMapTiming := 8.500p15;
s`SolvableDBLocalSanityCheckTiming := 0.930p15;
...
\[ 2, 5, 8, 9, 6, 1, 4, 13, 10, 7, 17, 16, 15, 18, 3, 14, 21, 12, 25, 26, 23, 24, 11, 20, 29, 22, 33, 32, 31, 34, 19, 30, 37, 28, 41, 42, 39, 40, 27, 36, 45, 38, 49, 48, 47, 50, 35, 46, 53, 44, 57, 58, 55, 56, 43, 52, 61, 54, 60, 64, 63, 59, 51, 62 ]:
 Order := 4 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,1,2-g0-path1", "4T2-2,2,2-g0-path2", "8T4-2,4,2-g0-path2", "16T13-2,8,2-g0-path1", "32S18-2,16,2-g0-path1", "64S53-4,32,2-g8-path1" ];
s`SolvableDBParents := [ Strings() | "128S148-4,32,4-g31-path8", "128S156-4,32,4-g31-path5", "128S147-4,32,2-g15-path5" ];
