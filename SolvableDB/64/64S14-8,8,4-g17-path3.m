...
I<[x]> := ideal< P | [
x[2]^2*x[4]^4 + 2*x[1]*x[4]^4 + 1/2*nu^4*x[1]^3 - x[2]^2,
x[1]^5 - x[2]^2 - x[1],
x[1]^3*x[3] - 1/8*nu^4*x[2]^2*x[4]^2 + 1/8*nu^4*x[2]^2*x[3] - 1/4*nu^4*x[1]*x[4]^2,
x[1]^2*x[3]^2 - x[1]^2 - 1/4*nu^4*x[3]^2 - 1/4*nu^4,
x[2]^2*x[3]^2 - 1/2*nu^4*x[1]^3 + 2*x[1]*x[3]^2 - x[2]^2,
x[1]^2*x[4]^2 - x[1]^2*x[3] + 1/4*nu^4*x[4]^2 + 1/4*nu^4*x[3],
x[3]*x[4]^2 - 1
] >;
...
s`SolvableDBName := "64S14-8,8,4-g17-path3";
s`SolvableDBFilename := "64S14-8,8,4-g17-path3.m";
...
{ IntegerRing() | 49, 62 }
@};
s`SolvableDBBelyiMapTiming := 43.940p15;
s`SolvableDBLocalSanityCheckTiming := 17.990p15;
...
\[ 6, 1, 27, 28, 22, 29, 24, 30, 2, 3, 4, 5, 7, 8, 64, 37, 16, 51, 40, 47, 33, 9, 54, 63, 44, 58, 53, 13, 12, 19, 61, 52, 48, 57, 10, 62, 60, 11, 14, 15, 17, 18, 20, 21, 23, 25, 26, 31, 32, 34, 35, 36, 42, 49, 41, 55, 43, 50, 56, 59, 46, 45, 38, 39 ]:
 Order := 8 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,1,2-g0-path1", "4T2-2,2,2-g0-path2", "8T4-2,4,2-g0-path2", "16T12-2,8,4-g2-path1", "32S10-4,8,4-g7-path7", "64S14-8,8,4-g17-path3" ];
s`SolvableDBParents := [ Strings() | "128S98-16,16,4-g41-path5", "128S98-16,16,4-g41-path6", "128S98-16,16,4-g41-path7", "128S98-16,16,4-g41-path8", "128S4-8,8,4-g33-path9", "128S28-8,8,4-g33-path11", "128S4-8,8,4-g33-path10" ];
