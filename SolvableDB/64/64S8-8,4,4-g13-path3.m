...
I<[x]> := ideal< P | [
x[3]^2*x[4]^4 + nu^2*x[4]^4 + (2*nu^2 + 2)*x[2]*x[3]^2 + (-2*nu^2 + 2)*x[1]*x[4]^2 + 3*nu^2*x[3]^2 + (-2*nu^2 - 2)*x[2] + 1,
x[1]*x[3]^2*x[4]^2 + x[2]*x[3]^2 - nu^2*x[1]*x[4]^2 + (nu^2 + 1)*x[3]^2 - x[2],
x[1]*x[4]^4 + (-nu^2 - 1)*x[1]^2*x[4]^2 + (nu^2 + 1)*x[4]^2 - nu^2*x[1],
x[1]^2*x[3]^2 - nu^2*x[1]^2 + x[3]^2 + nu^2,
x[1]*x[2]*x[3]^2 - x[3]^2*x[4]^2 + (nu^2 + 1)*x[1]*x[3]^2 - x[1]*x[2] - nu^2*x[4]^2,
x[2]^2*x[3]^2 - x[2]^2 - nu^2*x[3]^2 - nu^2,
x[1]^2*x[2] + (nu^2 - 1)*x[1]*x[4]^2 + x[1]^2 + nu^2*x[2] - 1,
x[2]*x[4]^2 - x[1]*x[2] - nu^2*x[4]^2 - x[1]
] >;
...
s`SolvableDBName := "64S8-8,4,4-g13-path3";
s`SolvableDBFilename := "64S8-8,4,4-g13-path3.m";
...
{ IntegerRing() | 46, 64 }
@};
s`SolvableDBBelyiMapTiming := 6.410p15;
s`SolvableDBLocalSanityCheckTiming := 2.270p15;
...
\[ 6, 1, 26, 27, 23, 28, 4, 29, 2, 3, 5, 7, 8, 22, 14, 56, 46, 39, 33, 36, 45, 51, 9, 20, 57, 18, 41, 11, 50, 19, 21, 59, 62, 55, 10, 12, 13, 15, 16, 17, 24, 25, 30, 31, 32, 34, 54, 47, 64, 42, 48, 60, 63, 38, 49, 35, 37, 52, 53, 43, 40, 58, 44, 61 ]:
 Order := 8 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T2-2,2,2-g0-path1", "8T4-4,2,2-g0-path1", "16T10-4,4,2-g1-path5", "32S6-4,4,4-g5-path9", "64S8-8,4,4-g13-path3" ];
s`SolvableDBParents := [ Strings() | "128S2-8,8,4-g33-path19", "128S3-8,8,4-g33-path70", "128S77-8,8,4-g33-path8", "128S78-8,8,4-g33-path18", "128S79-16,4,4-g29-path10", "128S81-16,8,4-g37-path3", "128S80-16,4,4-g29-path24", "128S82-16,8,4-g37-path8", "128S71-16,4,4-g29-path2", "128S73-16,8,4-g37-path1", "128S72-16,4,4-g29-path8", "128S74-16,8,4-g37-path4", "128S26-8,4,4-g25-path32", "128S75-8,4,4-g25-path6", "128S76-8,4,4-g25-path16" ];
