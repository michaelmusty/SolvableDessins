...
I<[x]> := ideal< P | [
x[4]^2*x[5]^4 + 1/4*(5*nu^3 + 25*nu - 10)*x[5]^4 + 1/8*(-nu^3 - 7*nu)*x[2]*x[4]^2 + 1/4*(5*nu^2 + 15)*x[3]*x[5]^2 + 1/16*(-nu^3 - 7*nu)*x[4]^2 + 1/32*(-5*nu^3 - 15*nu^2 - 35*nu - 45),
x[3]*x[4]^2*x[5]^2 + 1/8*(-nu^2 - 3)*x[1]*x[4]^2 + 1/8*(-nu^3 - 5*nu - 2)*x[2]*x[4]^2 + 1/4*(5*nu^3 + 25*nu + 10)*x[3]*x[5]^2 + 1/8*(-nu^3 - 5*nu)*x[4]^2 + 1/16*(-5*nu^3 - 25*nu - 10),
x[3]*x[5]^4 + 1/4*(-nu^2 - 3)*x[3]^2*x[5]^2 + 1/8*(nu^3 + 5*nu)*x[5]^2 + 1/16*(nu^3 + 7*nu)*x[3],
x[1]*x[2]*x[4]^2 + 1/4*(5*nu^3 + 35*nu)*x[2] + 1/4*(-5*nu^3 - 35*nu),
x[2]^2*x[4]^2 + x[2]*x[4]^2 + 1/4*(5*nu^3 + 35*nu)*x[1],
x[1]*x[3]*x[4]^2 + 1/4*(-nu^3 + nu^2 - 7*nu + 3)*x[2]*x[3]*x[4]^2 + 1/2*(-nu^3 - 5*nu)*x[4]^2*x[5]^2 + 1/4*(-nu^3 - 7*nu)*x[3]*x[4]^2 + 1/4*(5*nu^3 + 25*nu + 10)*x[5]^2 + 1/8*(5*nu^3 - 5*nu^2 + 35*nu - 15)*x[3],
x[3]^2*x[4]^2 - x[1]*x[4]^2 + 1/4*(-nu^3 - 7*nu)*x[2]*x[4]^2 + 1/2*(5*nu^3 + 35*nu)*x[3]*x[5]^2 + 1/4*(-5*nu^3 - 25*nu + 10)*x[3]^2 + 1/8*(-3*nu^3 - 21*nu)*x[4]^2 + 1/16*(-15*nu^3 - 5*nu^2 - 105*nu - 15),
x[1]*x[3]^2 + 1/2*(-nu^3 - 7*nu)*x[3]^2 + 1/8*(nu^3 + 7*nu)*x[1] - 1/2,
x[2]*x[3]^2 + (-nu^3 - 7*nu)*x[3]*x[5]^2 + 1/2*(nu^3 + 5*nu)*x[3]^2 + 1/8*(nu^3 + 7*nu)*x[2] + 1/8*(nu^2 + 3),
x[1]*x[5]^2 + 1/4*(-nu^2 - 3)*x[1]*x[3] - x[2]*x[3] + 1/2*(nu^3 + 7*nu)*x[5]^2,
x[2]*x[5]^2 - x[1]*x[3] + 1/4*(-nu^2 - 3)*x[2]*x[3] + 1/2*(nu^3 + 5*nu)*x[5]^2 + 1/4*(nu^3 + 7*nu)*x[3],
x[1]^2 - x[2]^2 + 1
] >;
...
s`SolvableDBName := "64S6-4,8,8-g17-path1";
s`SolvableDBFilename := "64S6-4,8,8-g17-path1.m";
...
{ IntegerRing() | 51, 63 }
@};
s`SolvableDBBelyiMapTiming := 613010.710p15;
s`SolvableDBLocalSanityCheckTiming := 123.520p15;
...
\[ 6, 1, 26, 27, 23, 9, 4, 28, 2, 3, 5, 7, 8, 50, 43, 52, 13, 63, 31, 44, 56, 29, 36, 21, 57, 33, 12, 17, 19, 20, 22, 18, 10, 38, 30, 11, 14, 15, 16, 24, 25, 32, 34, 35, 53, 61, 46, 47, 41, 51, 37, 60, 59, 64, 45, 40, 49, 54, 55, 39, 48, 58, 42, 62 ]:
 Order := 4 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T2-2,2,2-g0-path1", "8T4-2,2,4-g0-path1", "16T10-2,4,4-g1-path4", "32S11-4,4,8-g7-path2", "64S6-4,8,8-g17-path1" ];
s`SolvableDBParents := [ Strings() | "128S67-8,8,16-g45-path1", "128S68-8,8,16-g45-path1", "128S68-8,8,16-g45-path2", "128S67-8,8,16-g45-path2", "128S63-4,16,8-g37-path5", "128S61-8,16,16-g49-path19", "128S66-4,16,8-g37-path8", "128S62-8,16,16-g49-path11", "128S65-4,16,8-g37-path3", "128S62-8,16,16-g49-path12", "128S64-4,16,8-g37-path12", "128S61-8,16,16-g49-path20", "128S3-4,8,8-g33-path44", "128S2-4,8,8-g33-path11", "128S11-4,8,8-g33-path38" ];
s`SolvableDBChild := "32S11-4,4,8-g7-path2";
