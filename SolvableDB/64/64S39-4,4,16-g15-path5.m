...
phi := KX!((2*x[1]^9 + 10*x[1])*x[3]^9 + (-23*x[1]^16 - 96*x[1]^8 + 1)/(4*x[1]^8)*x[3]^8 + (7*x[1]^8 + 19)/x[1]*x[3]^7 + (-6*x[1]^8 - 6)/x[1]^2*x[3]^6 + (5*x[1]^8 + 1)/x[1]^3*x[3]^5 + (8*x[1]^12 + 36*x[1]^4)*x[3]^4 + (-7*x[1]^11 - 13*x[1]^3)*x[3]^3 + (6*x[1]^10 + 2*x[1]^2)*x[3]^2 + (2*x[1]^17 + 5*x[1]^9 + x[1])*x[3] + (9*x[1]^16 + 80*x[1]^8 + 1)/4);
/* assign to object */
...
s`SolvableDBLocalSanityCheckTiming := 4.770p15;
...
\[ 2, 5, 9, 7, 6, 1, 10, 11, 14, 21, 15, 16, 30, 22, 23, 31, 24, 25, 26, 12, 4, 3, 8, 19, 32, 17, 18, 33, 34, 28, 20, 27, 13, 38, 39, 47, 42, 41, 37, 43, 29, 35, 48, 40, 49, 50, 45, 44, 36, 53, 55, 63, 58, 57, 54, 59, 51, 46, 62, 56, 64, 60, 61, 52 ]:
 Order := 4 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,1,2-g0-path1", "4T2-2,2,2-g0-path2", "8T4-2,4,2-g0-path2", "16T10-2,4,4-g1-path5", "32S9-2,4,8-g3-path3", "64S39-4,4,16-g15-path5" ];
s`SolvableDBParents := [ Strings() | "128S94-4,8,16-g37-path5", "128S64-4,8,16-g37-path3", "128S97-4,8,16-g37-path5", "128S64-4,8,16-g37-path4", "128S80-4,4,16-g29-path5", "128S112-4,4,16-g29-path15", "128S80-4,4,16-g29-path6" ];
