...
x[2]^3*x[4]^2 + 2*x[1]*x[2]^2*x[3] + x[2]^3*x[3] + x[1]*x[4]^2 - x[1]*x[3] + 2*x[2]*x[3],
x[1]^2*x[2]^2 - x[1]^2 - x[2]^2 - 1,
x[1]^2*x[3]^2 + x[2]^2*x[3]^2 + x[1]^2 - 2*x[1]*x[2] + x[2]^2 + 2,
x[1]*x[2]*x[3]^2 + x[1]*x[2] + x[3]^2 - 1,
x[1]^2*x[4]^2 + x[2]^2*x[4]^2 + x[1]^2*x[3] + 2*x[1]*x[2]*x[3] + x[2]^2*x[3] + 2*x[3],
x[1]*x[2]*x[4]^2 + x[1]*x[2]*x[3] - x[4]^2 + x[3],
x[3]*x[4]^2 - 1
] >;
...
phi := KX!((-8*x[4]^16 - 8*x[4]^8)/(x[4]^24 - 2*x[4]^20 - x[4]^16 + 4*x[4]^12 - x[4]^8 - 2*x[4]^4 + 1)*x[2]^2 + (x[4]^24 - 2*x[4]^20 - x[4]^16 - 12*x[4]^12 - x[4]^8 - 2*x[4]^4 + 1)/(x[4]^24 - 2*x[4]^20 - x[4]^16 + 4*x[4]^12 - x[4]^8 - 2*x[4]^4 + 1));
/* assign to object */
...
s`SolvableDBName := "64S20-4,8,4-g13-path14";
s`SolvableDBFilename := "64S20-4,8,4-g13-path14.m";
...
\[ 22, 5, 54, 39, 6, 38, 4, 63, 12, 18, 24, 1, 11, 15, 29, 61, 50, 27, 40, 16, 47, 9, 35, 28, 45, 44, 51, 13, 55, 21, 23, 25, 26, 43, 59, 42, 7, 2, 37, 8, 33, 57, 3, 41, 62, 49, 56, 46, 52, 53, 10, 48, 60, 34, 14, 30, 58, 36, 31, 17, 64, 32, 19, 20 ]:
 Order := 4 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T2-2,2,2-g0-path1", "8T4-4,2,2-g0-path1", "16T10-4,2,4-g1-path5", "32S2-4,4,4-g5-path5", "64S20-4,8,4-g13-path14" ];
s`SolvableDBParents := [ Strings() | "128S11-8,8,4-g33-path13", "128S8-4,8,8-g33-path10", "128S10-8,8,8-g41-path43", "128S32-8,8,4-g33-path1", "128S35-4,8,8-g33-path4", "128S31-8,8,8-g41-path7", "128S27-8,8,4-g33-path13", "128S32-4,8,8-g33-path7", "128S34-8,8,8-g41-path7", "128S20-8,8,4-g33-path3", "128S17-4,8,8-g33-path6", "128S23-8,8,8-g41-path13", "128S29-4,8,4-g25-path1", "128S26-4,8,4-g25-path1", "128S16-4,8,4-g25-path3" ];
