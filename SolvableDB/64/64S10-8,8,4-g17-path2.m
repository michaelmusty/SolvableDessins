...
I<[x]> := ideal< P | [
x[3]^2*x[4]^4 + 1/2*x[2]*x[4]^4 + 1/4*x[1]*x[2]*x[3]^2 - x[3]^2*x[4]^2 - x[4]^4 - 1/2*x[1]*x[3]^2 - 1/4*x[2]*x[3]^2 + 1/2*x[2]*x[4]^2 - 1/4*x[1]^2 - 1/4*x[3]^2 - 3/4*x[1] + 1/8*x[2] + 1/4,
x[1]*x[2]^2*x[3]^2 + 1/2*x[1]*x[2]*x[3]^2 - 5/2*x[2]^2*x[3]^2 + 2*x[3]^2*x[4]^2 - x[1]^2*x[2] + x[1]*x[3]^2 - 3/2*x[2]*x[3]^2 + x[2]*x[4]^2 - 1/2*x[1]^2 - 3/2*x[1]*x[2] - 2*x[4]^2 - 3/2*x[1] + 5/2*x[2] + 1,
x[2]^3*x[3]^2 - 4*x[1]*x[2]*x[3]^2 - x[1]*x[2]^2 - 2*x[2]*x[3]^2 - 4*x[1]^2 - 4*x[2]^2 - 2*x[1],
x[2]*x[3]^2*x[4]^2 + 3/8*x[1]*x[2]*x[3]^2 - 1/8*x[2]^2*x[3]^2 - 1/2*x[3]^2*x[4]^2 - 1/4*x[1]*x[3]^2 - 1/8*x[2]*x[3]^2 - 1/4*x[2]*x[4]^2 - 3/8*x[1]^2 + 1/8*x[1]*x[2] - 1/2*x[4]^2 - 5/8*x[1] + 3/8*x[2] + 1/4,
x[1]^2*x[3]^2 + 1/2*x[1]*x[2]*x[3]^2 - 1/2*x[2]^2*x[3]^2 + 2*x[3]^2*x[4]^2 + x[1]*x[3]^2 - 3/2*x[2]*x[3]^2 + x[2]*x[4]^2 - 1/2*x[1]^2 + 1/2*x[1]*x[2] - x[3]^2 - 2*x[4]^2 - 3/2*x[1] + 3/2*x[2] + 1,
x[2]^2*x[4]^2 - 2*x[2]*x[4]^2 + x[1]^2 - x[1]*x[2] + 1/2*x[2]^2 + 2*x[4]^2 + x[1] - x[2] - 1,
x[1]^3 - x[2]^2 - x[1],
x[1]*x[4]^2 + x[2]*x[4]^2 - x[4]^2 - 1/2*x[1] + 1/2*x[2] + 1/2
] >;
...
s`SolvableDBName := "64S10-8,8,4-g17-path2";
s`SolvableDBFilename := "64S10-8,8,4-g17-path2.m";
...
s`SolvableDBLocalSanityCheckTiming := 1.410p15;
...
\[ 44, 61, 31, 14, 30, 22, 64, 11, 21, 20, 38, 28, 27, 36, 7, 19, 42, 45, 40, 47, 48, 2, 39, 41, 49, 56, 8, 51, 4, 37, 33, 53, 43, 5, 16, 62, 6, 63, 13, 59, 60, 52, 24, 9, 50, 55, 18, 12, 17, 26, 1, 32, 58, 3, 35, 10, 46, 25, 57, 54, 34, 15, 23, 29 ]:
 Order := 8 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,1,2-g0-path1", "4T2-2,2,2-g0-path2", "8T4-2,4,2-g0-path2", "16T10-4,4,2-g1-path4", "32S11-8,4,4-g7-path4", "64S10-8,8,4-g17-path2" ];
s`SolvableDBParents := [ Strings() | "128S87-16,16,4-g41-path3", "128S88-16,16,4-g41-path9", "128S87-16,16,4-g41-path4", "128S88-16,16,4-g41-path10", "128S4-8,8,4-g33-path5", "128S2-8,8,4-g33-path2", "128S27-8,8,4-g33-path5" ];
