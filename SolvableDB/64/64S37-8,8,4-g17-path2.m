...
I<[x]> := ideal< P | [
x[1]^2*x[4]^4 + 1/25*(-16*nu^3 - 416*nu^2 + 512*nu - 288)*x[1]^4*x[3] + 1/2*(-nu^3 - nu^2 - nu - 1)*x[3]*x[4]^4 + 1/25*(416*nu^3 - 512*nu^2 + 288*nu - 16)*x[1]^4 + 1/2*(nu^3 - nu^2 - nu - 1)*x[4]^4 + 1/25*(-800*nu^3 + 304*nu^2 + 400*nu - 928)*x[1]^2*x[3] + 1/25*(-560*nu^3 + 592*nu^2 - 480*nu + 256)*x[2]^2*x[3] + 1/25*(-720*nu^3 + 112*nu^2 + 640*nu - 784)*x[1]^2 + 1/25*(-560*nu^3 + 592*nu^2 - 480*nu + 256)*x[2]^2 + 1/25*(-616*nu^3 + 344*nu^2 - 488*nu + 392)*x[3] + 1/25*(-424*nu^3 + 648*nu^2 - 232*nu + 264),
x[2]^2*x[4]^4 + 1/25*(-288*nu^3 + 16*nu^2 + 416*nu - 512)*x[2]^4*x[3] + 1/2*(nu^3 + nu^2 - nu - 1)*x[3]*x[4]^4 + 1/25*(-288*nu^3 + 16*nu^2 + 416*nu - 512)*x[2]^4 + 1/2*(nu^3 + nu^2 + nu + 1)*x[4]^4 + 1/25*(64*nu^3 - 608*nu^2 + 752*nu - 144)*x[1]^2*x[3] + 1/25*(-800*nu^3 + 304*nu^2 + 400*nu - 928)*x[2]^2*x[3] + 1/25*(608*nu^3 - 752*nu^2 + 144*nu + 64)*x[1]^2 + 1/25*(-384*nu^3 - 208*nu^2 + 688*nu - 944)*x[2]^2 + 1/25*(-808*nu^3 - 104*nu^2 + 56*nu - 472)*x[3] + 1/25*(-152*nu^3 + 72*nu^2 + 664*nu - 504),
x[1]*x[3]*x[4]^4 - nu^2*x[1]*x[4]^4 + 1/25*(-96*nu^3 - 224*nu^2 + 272*nu - 432)*x[1]^3*x[3] + 1/5*(4*nu^3 + 16*nu^2 - 28*nu + 8)*x[2]*x[3]*x[4]^2 + 1/25*(224*nu^3 - 272*nu^2 + 432*nu - 96)*x[1]^3 + 1/5*(-28*nu^3 + 8*nu^2 - 4*nu - 16)*x[2]*x[4]^2 + 1/25*(144*nu^3 - 80*nu^2 + 192*nu - 240)*x[1]*x[3] + 1/25*(-144*nu^3 + 80*nu^2 - 192*nu + 240)*x[1],
x[2]*x[3]*x[4]^4 - nu^3*x[2]*x[4]^4 + 1/25*(224*nu^3 - 272*nu^2 + 432*nu - 96)*x[2]^3*x[3] + 1/5*(28*nu^3 - 8*nu^2 + 4*nu + 16)*x[1]*x[3]*x[4]^2 + 1/25*(224*nu^3 - 272*nu^2 + 432*nu - 96)*x[2]^3 + 1/5*(16*nu^3 - 28*nu^2 + 8*nu - 4)*x[1]*x[4]^2 + 1/25*(-192*nu^3 + 240*nu^2 + 144*nu - 80)*x[2]*x[3] + 1/25*(240*nu^3 + 144*nu^2 - 80*nu + 192)*x[2],
x[3]^2*x[4]^4 + (-nu^3 - nu^2)*x[3]*x[4]^4 - nu*x[4]^4 + 1/25*(656*nu^3 - 368*nu^2 + 208*nu + 176)*x[1]^2*x[3] + 1/25*(-208*nu^3 - 176*nu^2 + 656*nu - 368)*x[2]^2*x[3] + 1/25*(368*nu^3 - 208*nu^2 - 176*nu + 656)*x[1]^2 + 1/25*(-208*nu^3 - 176*nu^2 + 656*nu - 368)*x[2]^2 + 1/25*(192*nu^3 + 304*nu^2 + 256*nu - 128)*x[3]^2 + 1/25*(-96*nu^3 - 224*nu^2 + 272*nu - 432)*x[3] + 1/25*(-16*nu^3 - 416*nu^2 + 512*nu - 288),
x[1]^2*x[3]*x[4]^2 + nu*x[1]^2*x[4]^2 + 1/5*(-24*nu^3 - 16*nu^2 - 12*nu + 12)*x[1]*x[2]*x[3] + (-nu^3 - nu^2 + 1)*x[3]*x[4]^2 + 1/5*(-24*nu^3 - 16*nu^2 - 12*nu + 12)*x[1]*x[2] + (-nu^3 - nu^2 - nu)*x[4]^2,
x[2]^2*x[3]*x[4]^2 - x[2]^2*x[4]^2 + 1/5*(-16*nu^3 - 12*nu^2 + 12*nu + 24)*x[1]*x[2]*x[3] + (-nu^3 - nu^2 + 1)*x[3]*x[4]^2 + 1/5*(12*nu^3 - 12*nu^2 - 24*nu - 16)*x[1]*x[2] + (-nu^2 - nu - 1)*x[4]^2,
x[1]*x[3]^2*x[4]^2 + (-nu^2 + nu)*x[1]*x[3]*x[4]^2 + 1/5*(8*nu^3 + 4*nu^2 - 16*nu - 8)*x[2]*x[3]^2 - nu^3*x[1]*x[4]^2 + 1/5*(-4*nu^2 - 20*nu + 8)*x[2]*x[3] + 1/5*(-8*nu^3 - 8*nu^2 - 4*nu + 16)*x[2],
x[2]*x[3]^2*x[4]^2 + (-nu^3 - 1)*x[2]*x[3]*x[4]^2 + 1/5*(16*nu^3 + 8*nu^2 + 8*nu + 4)*x[1]*x[3]^2 + nu^3*x[2]*x[4]^2 + 1/5*(-4*nu^2 - 20*nu + 8)*x[1]*x[3] + 1/5*(-4*nu^3 + 16*nu^2 + 8*nu + 8)*x[1],
x[1]^2*x[2]^2 - x[1]^2 + nu^2*x[2]^2 + nu^2,
x[1]^2*x[2]*x[3] + 1/68*(-16*nu^3 - 9*nu^2 + 13*nu + 2)*x[1]*x[3]*x[4]^2 - nu*x[1]^2*x[2] + 1/68*(-13*nu^3 - 2*nu^2 - 16*nu - 9)*x[1]*x[4]^2 + nu^3*x[2]*x[3] - nu^2*x[2],
x[1]*x[2]^2*x[3] + 1/68*(-9*nu^3 + 13*nu^2 + 2*nu + 16)*x[2]*x[3]*x[4]^2 + x[1]*x[2]^2 + 1/68*(-16*nu^3 - 9*nu^2 + 13*nu + 2)*x[2]*x[4]^2 + nu^3*x[1]*x[3] - nu*x[1],
x[1]^2*x[3]^2 - nu^2*x[1]^2 + x[3]^2 + nu^2,
x[1]*x[2]*x[3]^2 + 1/68*(-18*nu^3 + 9*nu^2 + 4*nu - 2)*x[3]^2*x[4]^2 + (-nu + 1)*x[1]*x[2]*x[3] + 1/68*(-2*nu^3 - 16*nu^2 - 9*nu + 13)*x[3]*x[4]^2 - nu*x[1]*x[2] + 1/68*(-9*nu^3 - 4*nu^2 + 2*nu - 18)*x[4]^2,
x[2]^2*x[3]^2 - x[2]^2 - nu^2*x[3]^2 - nu^2,
x[1]*x[2]*x[4]^2 + 1/5*(-4*nu^3 + 2*nu^2 + 8*nu - 14)*x[1]^2*x[3] + 1/5*(-14*nu^3 + 4*nu^2 - 2*nu - 8)*x[2]^2*x[3] + 1/5*(-2*nu^3 - 8*nu^2 + 14*nu - 4)*x[1]^2 + 1/5*(-14*nu^3 + 4*nu^2 - 2*nu - 8)*x[2]^2 + 1/5*(-2*nu^3 + 10*nu^2 - 6*nu - 10)*x[3] + 1/5*(-16*nu^3 - 8*nu^2 - 8*nu - 4)
] >;
...
s`SolvableDBName := "64S37-8,8,4-g17-path2";
s`SolvableDBFilename := "64S37-8,8,4-g17-path2.m";
...
{ IntegerRing() | 51, 56 }
@};
s`SolvableDBBelyiMapTiming := 135.740p15;
s`SolvableDBLocalSanityCheckTiming := 14.230p15;
...
\[ 6, 1, 27, 28, 22, 29, 24, 30, 2, 3, 4, 5, 7, 8, 60, 26, 34, 31, 11, 51, 43, 9, 36, 44, 64, 42, 21, 13, 12, 62, 38, 54, 61, 59, 47, 48, 63, 10, 14, 15, 16, 17, 18, 19, 20, 23, 25, 32, 33, 35, 37, 57, 45, 52, 56, 53, 46, 41, 58, 39, 50, 40, 55, 49 ]:
 Order := 8 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T2-2,2,2-g0-path1", "8T4-4,2,2-g0-path1", "16T10-4,4,2-g1-path5", "32S6-4,4,4-g5-path9", "64S37-8,8,4-g17-path2" ];
s`SolvableDBParents := [ Strings() | "128S86-8,8,4-g33-path7", "128S78-8,8,4-g33-path7", "128S126-8,8,4-g33-path10", "128S57-8,8,4-g33-path4", "128S78-8,8,4-g33-path8", "128S86-8,8,4-g33-path8", "128S51-8,8,4-g33-path4" ];
