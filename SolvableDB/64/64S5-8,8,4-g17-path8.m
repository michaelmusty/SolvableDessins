...
I<[x]> := ideal< P | [
x[1]^2*x[4]^4 + (-nu^3 + 2*nu^2 - nu)*x[1]^4*x[3] + nu*x[1]*x[4]^4 + (2*nu^3 - nu^2 + 1)*x[1]^4 + (-2*nu^3 + 3*nu^2 - 4*nu + 1)*x[1]^3*x[3] + (nu^3 - nu^2 - nu + 1)*x[2]*x[3]*x[4]^2 + (3*nu^3 - 4*nu^2 + nu + 2)*x[1]^3 + (-nu^3 + nu - 2)*x[1]*x[2]^2 + (-2*nu^3 + 6*nu^2 - 4*nu + 2)*x[1]^2*x[3] + (4*nu^3 - 3*nu^2 + 2*nu + 1)*x[1]^2 + (-2*nu^3 + nu^2 - 1)*x[2]^2 + (-2*nu^3 + 3*nu^2 - 4*nu + 1)*x[1]*x[3] + (nu^3 - nu + 2)*x[1] + (-nu^3 + 2*nu^2 - nu)*x[3] - 2*nu^3 + 2*nu^2 - 2*nu,
x[2]^2*x[4]^4 + (nu^3 - nu + 2)*x[1]*x[2]^4 - nu^3*x[1]*x[4]^4 + (2*nu^3 - nu^2 + 1)*x[2]^4 + (-nu^2 + 2*nu - 1)*x[1]^3*x[3] + (-3*nu^3 + 3*nu^2 + nu - 1)*x[2]*x[3]*x[4]^2 + x[4]^4 + (-nu^3 + 2*nu^2 - nu)*x[1]^3 + (nu^3 - 4*nu^2 + 3*nu - 2)*x[1]*x[2]^2 + (-2*nu^3 + 2*nu - 4)*x[1]^2*x[3] + (-4*nu^2 + 4*nu)*x[2]*x[4]^2 + (2*nu^2 - 4*nu + 2)*x[1]^2 + (-2*nu^2 + 4*nu - 2)*x[2]^2 + (-nu^2 + 2*nu - 1)*x[1]*x[3] + (-2*nu^3 + 4*nu^2 - 2*nu)*x[1] + (2*nu^2 - 2*nu + 2)*x[3] - nu^2 + 2*nu - 1,
x[2]*x[3]*x[4]^4 - nu^3*x[2]*x[4]^4 + (-2*nu^3 + nu^2 - 1)*x[1]*x[2]^3 + (-nu^3 + nu^2 - nu + 1)*x[2]^2*x[4]^2 + (-nu^3 + nu^2)*x[3]^2*x[4]^2 + (-nu^3 + 2*nu^2 - nu)*x[2]^3 + (nu^3 - 2*nu^2 + nu)*x[2]*x[3]^2 + (2*nu^2 - 2*nu)*x[1]*x[4]^2 + (-nu^3 - nu^2 + nu - 1)*x[3]*x[4]^2 + (2*nu^3 - nu^2 + 1)*x[1]*x[2] + (-nu^2 + 2*nu - 1)*x[2]*x[3] + (-nu + 1)*x[4]^2 + (-nu^3 + 2*nu^2 - nu)*x[2],
x[3]^2*x[4]^4 + (nu^3 + nu)*x[1]*x[4]^4 + (2*nu^3 - 2*nu + 2)*x[1]^3*x[3] + (-2*nu + 2)*x[2]*x[3]*x[4]^2 + nu^2*x[4]^4 + (-2*nu^2 + 2*nu - 2)*x[1]^3 + (-2*nu^3 + 2*nu^2 - 2)*x[1]*x[2]^2 + (2*nu^3 - 2*nu + 4)*x[1]^2*x[3] + (-2*nu^2 + 4*nu - 2)*x[1]^2 + (-2*nu^3 + 2*nu^2 - 2*nu)*x[2]^2 + (2*nu^3 - 2*nu + 2)*x[1]*x[3] + (-nu^2 + 2*nu - 1)*x[3]^2 + (2*nu^3 - 2*nu^2 + 2)*x[1] + nu^2 - 2*nu + 1,
x[2]^2*x[3]*x[4]^2 + 1/2*(-nu^3 + nu^2 - nu - 1)*x[1]*x[2]^3 + 1/2*(nu^3 - nu)*x[3]^2*x[4]^2 + 1/2*(-nu^3 + nu^2 - nu + 1)*x[2]^3 + 1/2*(-nu^3 + nu^2 + nu - 1)*x[2]*x[3]^2 + 1/2*(nu^3 - nu^2 + nu + 1)*x[1]*x[2] + 1/2*(nu^3 - nu)*x[4]^2,
x[2]*x[3]^2*x[4]^2 + (-nu^3 - nu)*x[2]*x[3]*x[4]^2 + (nu - 1)*x[1]*x[2]^2 - x[2]*x[4]^2 + (-nu^3 - 1)*x[2]^2 + (-nu^2 + nu)*x[3]^2 + (-nu + 1)*x[1],
x[3]^3*x[4]^2 + (-nu^3 - 1)*x[2]*x[3]^3 - nu^3*x[3]^2*x[4]^2 + (nu^3 - nu^2 + nu - 1)*x[1]*x[2]*x[3] + (-nu + 1)*x[2]*x[3]^2 + x[3]*x[4]^2 + (-nu^3 + nu^2 - nu - 1)*x[1]*x[2] + (-nu^2 + nu)*x[2]*x[3] - nu^3*x[4]^2 + (-nu^3 + nu^2)*x[2],
x[1]^2*x[2]^2 - x[1]^2 + nu^2*x[2]^2 + nu^2,
x[1]^2*x[2]*x[3] + 1/2*(nu^3 + nu^2 - nu - 1)*x[3]^2*x[4]^2 + nu*x[1]^2*x[2] + nu*x[2]*x[3]^2 + (-nu^2 - nu)*x[1]*x[4]^2 + nu^2*x[2]*x[3] + 1/2*(-nu^3 - nu^2 - nu - 1)*x[4]^2 + (nu^3 - nu)*x[2],
x[1]*x[2]^2*x[3] + (-nu - 1)*x[2]*x[3]*x[4]^2 + nu*x[1]*x[2]^2 + nu^3*x[2]^2*x[3] + (nu^3 - 1)*x[2]*x[4]^2 - x[2]^2 - nu^2*x[1]*x[3] - nu^3*x[1] + nu^3*x[3] - 1,
x[1]^2*x[3]^2 - nu^2*x[1]^2 + x[3]^2 + nu^2,
x[2]^2*x[3]^2 - x[2]^2 - nu^2*x[3]^2 - nu^2,
x[1]*x[2]*x[4]^2 + x[2]*x[3]*x[4]^2 + 1/2*(-nu^3 + nu^2 - nu - 1)*x[1]*x[2]^2 + 1/2*(nu^3 + nu^2 - nu + 1)*x[1]^2*x[3] - nu^3*x[2]*x[4]^2 + 1/2*(nu^3 - nu^2 + nu - 1)*x[1]^2 + 1/2*(-nu^3 + nu^2 - nu + 1)*x[2]^2 + 1/2*(nu^3 - nu^2 + nu + 1)*x[1] + 1/2*(nu^3 - nu^2 + nu + 1)*x[3],
x[1]*x[3]*x[4]^2 + nu^2*x[3]^2*x[4]^2 + (-nu^3 - 1)*x[1]*x[2]*x[3] + (-nu^2 + nu)*x[2]*x[3]^2 - nu*x[1]*x[4]^2 + nu*x[3]*x[4]^2 + (-nu + 1)*x[1]*x[2] + (-nu^3 + nu^2)*x[2]*x[3] + (nu^3 + nu^2 - nu + 1)*x[2]
] >;
...
{ IntegerRing() | 47, 50 }
@};
s`SolvableDBBelyiMapTiming := 3734.970p15;
s`SolvableDBLocalSanityCheckTiming := 61.890p15;
