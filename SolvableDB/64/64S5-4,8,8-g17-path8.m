...
I<[x]> := ideal< P | [
x[3]^2*x[4]^4 - 2*x[3]^2*x[4]^2 + x[4]^4 - 2*x[1]^2 + 2*x[3]^2,
x[1]*x[4]^4 - x[1]^3 - x[2]*x[4]^2 + x[1],
x[2]^2*x[3]^2 - x[3]^2*x[4]^2 + 2*x[1]*x[2] + x[2]^2 + x[3]^2 - x[4]^2 - 1,
x[1]*x[2]*x[4]^2 - x[1]^2 + 1,
x[2]^2*x[4]^2 - 2*x[1]*x[2] + x[4]^2,
x[1]^2*x[2] - x[1]*x[4]^2 + x[2],
...
s`SolvableDBName := "64S5-4,8,8-g17-path8";
s`SolvableDBFilename := "64S5-4,8,8-g17-path8.m";
...
{ IntegerRing() | 52, 57 }
@};
s`SolvableDBBelyiMapTiming := 3.590p15;
s`SolvableDBLocalSanityCheckTiming := 19.730p15;
...
\[ 6, 1, 27, 28, 24, 9, 4, 29, 2, 3, 5, 7, 8, 18, 47, 58, 13, 52, 32, 54, 59, 30, 57, 38, 21, 55, 34, 12, 17, 19, 20, 22, 23, 10, 37, 40, 51, 11, 14, 15, 16, 25, 26, 31, 33, 35, 36, 53, 64, 43, 46, 39, 60, 44, 50, 48, 45, 61, 42, 56, 41, 63, 49, 62 ]:
 Order := 4 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,1,2-g0-path1", "4T2-2,2,2-g0-path2", "8T4-2,4,2-g0-path2", "16T10-2,4,4-g1-path5", "32S6-4,4,4-g5-path3", "64S5-4,8,8-g17-path8" ];
s`SolvableDBParents := [ Strings() | "128S55-4,16,16-g41-path5", "128S60-4,16,16-g41-path1", "128S54-4,16,16-g41-path9", "128S59-4,16,16-g41-path1", "128S54-4,16,16-g41-path10", "128S59-4,16,16-g41-path2", "128S55-4,16,16-g41-path6", "128S60-4,16,16-g41-path2", "128S57-4,8,8-g33-path5", "128S3-4,8,8-g33-path25", "128S58-4,8,8-g33-path9", "128S3-4,8,8-g33-path26", "128S58-4,8,8-g33-path10", "128S12-4,8,8-g33-path19", "128S56-4,8,8-g33-path5" ];
