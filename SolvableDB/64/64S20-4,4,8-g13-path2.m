...
I<[x]> := ideal< P | [
x[1]^4*x[3]^2 + 16*x[1]^4 + 4*x[1]^2*x[3]^2 + 1/2*x[2]^2*x[3]^2 - 32*x[1]^2*x[2] + 2*x[2]*x[3]^2 + 8*x[2]^2 + 6*x[3]^2 - 32*x[2] - 32,
x[1]^4*x[2] + x[2]^2 + 4,
x[1]^2*x[2]*x[3]^2 - 2*x[1]^2*x[3]^2 + 16*x[1]^2*x[2] - 2*x[2]*x[3]^2 - 32*x[1]^2 - 4*x[3]^2 + 32*x[2] + 64,
x[2]^3*x[3]^2 - 64*x[1]^2*x[2]^2 + 16*x[1]^2*x[3]^2 + 2*x[2]^2*x[3]^2 - 128*x[1]^2*x[2] + 16*x[2]^3 + 28*x[2]*x[3]^2 + 256*x[1]^2 - 96*x[2]^2 + 24*x[3]^2 - 320*x[2] - 640
] >;
...
s`SolvableDBName := "64S20-4,4,8-g13-path2";
s`SolvableDBFilename := "64S20-4,4,8-g13-path2.m";
...
s`SolvableDBLocalSanityCheckTiming := 6.900p15;
...
\[ 11, 30, 12, 7, 2, 5, 10, 33, 18, 31, 9, 22, 51, 13, 36, 37, 8, 1, 24, 3, 25, 20, 14, 34, 35, 56, 29, 39, 59, 6, 4, 17, 32, 57, 47, 49, 52, 62, 40, 46, 38, 16, 60, 26, 27, 28, 21, 15, 48, 54, 23, 42, 41, 55, 64, 58, 19, 44, 45, 61, 63, 53, 43, 50 ]:
 Order := 4 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T2-2,2,2-g0-path1", "8T4-2,2,4-g0-path1", "16T10-4,2,4-g1-path4", "32S11-4,2,8-g3-path3", "64S20-4,4,8-g13-path2" ];
s`SolvableDBParents := [ Strings() | "128S11-8,4,8-g33-path14", "128S35-4,8,8-g33-path2", "128S31-8,8,8-g41-path3", "128S32-8,4,8-g33-path3", "128S8-4,8,8-g33-path8", "128S10-8,8,8-g41-path39", "128S27-8,4,8-g33-path14", "128S17-4,8,8-g33-path4", "128S23-8,8,8-g41-path9", "128S20-8,4,8-g33-path4", "128S32-4,8,8-g33-path3", "128S34-8,8,8-g41-path3", "128S29-4,4,8-g25-path3", "128S26-4,4,8-g25-path3", "128S16-4,4,8-g25-path5" ];
