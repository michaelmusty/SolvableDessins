...
I<[x]> := ideal< P | [
x[3]^2*x[4]^4 - x[1]*x[4]^4 - 1/2*x[1]*x[2]*x[4]^2 + 1/2*x[1]^2 + 1/4*x[3]^2 - 1/4,
x[1]*x[3]^2*x[4]^2 - x[3]^2*x[4]^2 + 1/2*x[2]*x[3]^2 + x[1]*x[4]^2 + x[4]^2 - 1/2*x[2],
x[2]*x[3]^2*x[4]^2 - x[1]*x[2]*x[4]^2 - 1/2*x[1]*x[3]^2 + x[1]^2 + 1/2*x[3]^2 - 1/2*x[1] - 1/2,
x[1]^2*x[3]^2 + 2*x[1]*x[3]^2 - x[1]^2 - x[3]^2 + 2*x[1] + 1,
x[1]*x[2]*x[3]^2 - 4*x[3]^2*x[4]^2 + 3*x[2]*x[3]^2 + 4*x[1]*x[4]^2 - x[1]*x[2] + x[2],
x[2]^2*x[3]^2 - 4*x[1]*x[3]^2 + 4*x[1]^2 - x[2]^2 + 2*x[3]^2 - 4*x[1] - 2,
x[1]^2*x[4]^2 + x[4]^2 - x[2],
x[2]^2*x[4]^2 + 2*x[1]*x[4]^2 - x[1]*x[2],
x[1]^3 - x[2]^2 - x[1]
] >;
...
s`SolvableDBName := "64S25-8,8,4-g17-path5";
s`SolvableDBFilename := "64S25-8,8,4-g17-path5.m";
...
s`SolvableDBLocalSanityCheckTiming := 0.800p15;
...
\[ 6, 1, 27, 28, 24, 29, 4, 30, 2, 3, 5, 7, 8, 62, 53, 48, 45, 34, 33, 64, 38, 51, 57, 9, 21, 63, 55, 44, 11, 17, 19, 20, 22, 23, 10, 47, 41, 12, 13, 14, 15, 16, 18, 25, 26, 31, 32, 35, 36, 37, 59, 50, 60, 46, 42, 61, 40, 52, 54, 58, 49, 43, 39, 56 ]:
 Order := 8 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,1,2-g0-path1", "4T2-2,2,2-g0-path2", "8T4-2,4,2-g0-path2", "16T10-4,4,2-g1-path4", "32S2-4,4,4-g5-path7", "64S25-8,8,4-g17-path5" ];
s`SolvableDBParents := [ Strings() | "128S33-8,8,8-g41-path54", "128S39-8,8,8-g41-path15", "128S13-8,8,8-g41-path24", "128S19-8,8,8-g41-path24", "128S15-8,8,8-g41-path60", "128S23-8,8,8-g41-path30", "128S34-8,8,8-g41-path36", "128S40-8,8,8-g41-path6", "128S18-8,8,4-g33-path16", "128S32-8,8,4-g33-path14", "128S38-8,8,4-g33-path5", "128S35-8,8,4-g33-path20", "128S37-8,8,4-g33-path14", "128S14-8,8,4-g33-path20", "128S20-8,8,4-g33-path10" ];
