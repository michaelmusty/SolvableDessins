...
I<[x]> := ideal< P | [
x[2]^3*x[4]^4 - nu*x[2]^2*x[4]^4 + (nu^2 - 1)*x[1]*x[2]^4 + (nu^3 - 3*nu)*x[2]^2*x[3]*x[4]^2 + nu^2*x[2]*x[4]^4 + 2*nu^3*x[1]*x[2]^3 + 3/2*nu^2*x[2]^4 + (-2*nu^3 + 2*nu)*x[1]*x[3]*x[4]^2 + (2*nu^2 + 2)*x[2]*x[3]*x[4]^2 - nu^3*x[4]^4 + (nu^2 - 1)*x[1]*x[2]^2 + 1/2*(5*nu^3 + 2*nu)*x[2]^3 + (nu^3 - nu)*x[3]*x[4]^2 + 1/2*(4*nu^2 - 3)*x[2]^2 + 1/2*(2*nu^3 - nu)*x[2],
x[2]^3*x[3]*x[4]^2 - x[1]*x[2]^4 - nu*x[2]^2*x[3]*x[4]^2 - nu*x[1]*x[2]^3 + 1/2*(nu^2 - 1)*x[2]^4 + nu^2*x[2]*x[3]*x[4]^2 + x[1]*x[2]^2 + 1/2*(nu^3 - nu)*x[2]^3 - nu^3*x[3]*x[4]^2 + nu*x[1]*x[2] + 1/2*(nu^2 + 1)*x[2]^2 + 1/2*(nu^3 + nu)*x[2],
x[1]^2*x[4]^4 - nu^2*x[2]^2*x[4]^4 + (nu^2 + 1)*x[1]*x[2]^3 - 4*x[1]*x[3]*x[4]^2 + 4*nu^3*x[2]*x[3]*x[4]^2 + 3/2*nu^2*x[1]^2*x[2] + 4*nu*x[1]*x[2]^2 + 3/2*x[2]^3 + 4*x[3]*x[4]^2 + (nu^3 + nu)*x[1]^2 + (3*nu^2 + 1)*x[1]*x[2] + (-2*nu^3 + 4*nu)*x[2]^2 + (2*nu^2 + 2)*x[2] - nu^3 + nu,
x[1]*x[2]^3*x[3] - x[2]^3*x[4]^2 + nu*x[1]*x[2]^2*x[3] + 1/2*(-nu^2 + 1)*x[2]^3*x[3] + nu*x[2]^2*x[4]^2 - x[1]*x[2]*x[3] + 1/2*(-nu^3 + nu)*x[2]^2*x[3] - nu^2*x[2]*x[4]^2 - nu*x[1]*x[3] + 1/2*(-nu^2 - 1)*x[2]*x[3] + nu^3*x[4]^2 + 1/2*(-nu^3 - nu)*x[3],
x[1]^2*x[3]*x[4]^2 - nu^2*x[2]^2*x[3]*x[4]^2 + nu^2*x[1]*x[2]^3 + 1/2*(nu^2 - 1)*x[1]^2*x[2] + 2*nu^3*x[1]*x[2]^2 + 1/2*(nu^2 + 1)*x[2]^3 + nu^3*x[1]^2 - x[1]*x[2] + (nu^3 + nu)*x[2]^2 + nu,
x[1]^2*x[2]^2 - x[1]^2 + nu^2*x[2]^2 + nu^2,
x[1]^2*x[2]*x[3] - nu^3*x[1]*x[2]^2*x[3] - nu*x[1]^2*x[4]^2 + nu^3*x[2]^2*x[4]^2 + 1/2*(-nu^3 + nu)*x[1]^2*x[3] + 2*x[1]*x[2]*x[3] + 1/2*(-nu^3 - nu)*x[2]^2*x[3] + nu*x[1]*x[3] + x[2]*x[3],
x[1]*x[2]*x[4]^2 + 1/2*(nu^2 + 1)*x[1]*x[2]*x[3] - nu*x[1]*x[4]^2 - x[2]*x[4]^2 + 1/2*(nu^3 + nu)*x[1]*x[3] + 1/2*(nu^2 + 1)*x[2]*x[3] + nu*x[4]^2 + 1/2*(nu^3 + nu)*x[3],
x[3]^2 - x[2]
] >;
...
s`SolvableDBName := "64S41-4,16,4-g15-path4";
s`SolvableDBFilename := "64S41-4,16,4-g15-path4.m";
...
{ IntegerRing() | 63, 64 }
@};
s`SolvableDBBelyiMapTiming := 1.540p15;
s`SolvableDBLocalSanityCheckTiming := 8.610p15;
...
\[ 19, 5, 33, 23, 6, 29, 4, 22, 12, 16, 8, 1, 11, 45, 25, 21, 34, 10, 9, 32, 18, 13, 28, 3, 20, 24, 31, 7, 2, 37, 17, 15, 26, 27, 61, 49, 43, 50, 44, 46, 30, 48, 41, 14, 39, 38, 53, 36, 42, 40, 56, 51, 59, 62, 47, 64, 60, 63, 55, 35, 57, 58, 54, 52 ]:
 Order := 4 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T2-2,2,2-g0-path1", "8T4-4,2,2-g0-path1", "16T10-4,4,2-g1-path5", "32S9-4,8,2-g3-path3", "64S41-4,16,4-g15-path4" ];
s`SolvableDBParents := [ Strings() | "128S92-8,16,4-g37-path5", "128S95-8,16,4-g37-path10", "128S65-8,16,4-g37-path1", "128S66-8,16,4-g37-path2", "128S118-4,16,4-g29-path16", "128S79-4,16,4-g29-path8", "128S80-4,16,4-g29-path16" ];
