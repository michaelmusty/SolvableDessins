...
I<[x]> := ideal< P | [
x[2]^4*x[4]^4 + 2*x[1]*x[2]^5 + x[2]^6 - 2*x[1]*x[2]^3 - 2*x[2]^4 + x[4]^4 - x[2]^2,
x[1]^3*x[2]*x[4]^2 - x[1]^3*x[3] - x[2]^3*x[3] - 2*x[1]^2*x[4]^2 + 3*x[1]*x[2]*x[4]^2 - x[2]^2*x[4]^2 + x[1]*x[3] + x[2]*x[3] + x[4]^2,
x[2]^4*x[3] + x[1]^2*x[2]*x[4]^2 - 2*x[1]*x[2]^2*x[4]^2 + x[2]^3*x[4]^2 - x[1]^2*x[3] + x[2]^2*x[3] + x[3],
x[1]*x[4]^4 - x[2]*x[4]^4 - x[1]*x[2]^2 - x[2]^3,
x[1]^2*x[2]^2 - x[1]^2 + x[2]^2 + 1,
x[1]*x[2]*x[3] + x[2]^2*x[3] - x[1]*x[4]^2 + x[2]*x[4]^2,
...
phi := KX!((-8*x[3]^10 - 8*x[3]^6)/(x[3]^12 - 2*x[3]^10 - x[3]^8 + 4*x[3]^6 - x[3]^4 - 2*x[3]^2 + 1)*x[4]^4 + 16*x[3]^6/(x[3]^12 - 2*x[3]^10 - x[3]^8 + 4*x[3]^6 - x[3]^4 - 2*x[3]^2 + 1));
/* assign to object */
...
s`SolvableDBName := "64S25-8,4,8-g17-path3";
s`SolvableDBFilename := "64S25-8,4,8-g17-path3.m";
...
\[ 6, 1, 27, 28, 22, 29, 24, 30, 2, 3, 4, 5, 7, 8, 43, 61, 57, 62, 14, 16, 47, 9, 52, 63, 31, 44, 34, 13, 12, 55, 50, 37, 59, 42, 64, 41, 35, 10, 11, 15, 17, 18, 19, 20, 21, 23, 25, 26, 32, 33, 36, 49, 60, 48, 40, 51, 53, 54, 45, 56, 58, 38, 39, 46 ]:
 Order := 8 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,1,2-g0-path1", "4T2-2,2,2-g0-path2", "8T4-2,4,2-g0-path2", "16T10-2,4,4-g1-path5", "32S2-4,4,4-g5-path3", "64S25-8,4,8-g17-path3" ];
s`SolvableDBParents := [ Strings() | "128S13-8,8,8-g41-path21", "128S19-8,8,8-g41-path21", "128S33-8,8,8-g41-path51", "128S39-8,8,8-g41-path12", "128S34-8,8,8-g41-path33", "128S40-8,8,8-g41-path3", "128S15-8,8,8-g41-path57", "128S23-8,8,8-g41-path27", "128S18-8,4,8-g33-path15", "128S32-8,4,8-g33-path13", "128S38-8,4,8-g33-path4", "128S35-8,4,8-g33-path19", "128S37-8,4,8-g33-path13", "128S14-8,4,8-g33-path19", "128S20-8,4,8-g33-path9" ];
