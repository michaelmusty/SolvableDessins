...
I<[x]> := ideal< P | [
x[3]^2*x[4]^4 + x[4]^4 + (-2*nu^3 - 2*nu)*x[2]*x[4]^2 + nu^2*x[3]^2 + nu^2,
x[2]*x[3]^2*x[4]^2 - nu^2*x[1]*x[3]^2 - x[2]*x[4]^2 - x[1] - nu^3 + nu,
x[2]*x[4]^4 + (-nu^3 - nu)*x[2]^2*x[4]^2 + (nu^3 - nu)*x[4]^2 + nu^2*x[2],
x[1]^2*x[3]^2 - nu^2*x[1]^2 + x[3]^2 + nu^2,
x[1]*x[2]*x[3]^2 - x[3]^2*x[4]^2 - nu^2*x[1]*x[2] - x[4]^2 + (nu^3 + nu)*x[2],
x[2]^2*x[3]^2 - x[2]^2 - nu^2*x[3]^2 - nu^2,
x[1]*x[2]^2 + (-nu^2 - 1)*x[2]*x[4]^2 + nu^3*x[2]^2 - x[1] + nu,
x[1]*x[4]^2 - nu*x[1]*x[2] + nu*x[4]^2 + x[2]
] >;
...
s`SolvableDBName := "64S8-4,8,4-g13-path4";
s`SolvableDBFilename := "64S8-4,8,4-g13-path4.m";
...
{ IntegerRing() | 45, 49 }
@};
s`SolvableDBBelyiMapTiming := 5.670p15;
...
\[ 2, 9, 10, 7, 11, 1, 12, 13, 6, 18, 35, 27, 36, 37, 38, 39, 40, 26, 29, 24, 30, 31, 5, 41, 42, 3, 4, 8, 43, 32, 15, 44, 19, 45, 23, 28, 51, 22, 53, 34, 57, 63, 33, 21, 17, 55, 16, 62, 50, 48, 56, 60, 47, 59, 52, 14, 20, 25, 64, 46, 54, 49, 58, 61 ]:
 Order := 4 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T2-2,2,2-g0-path1", "8T4-4,2,2-g0-path1", "16T10-4,4,2-g1-path5", "32S6-4,4,4-g5-path9", "64S8-4,8,4-g13-path4" ];
s`SolvableDBParents := [ Strings() | "128S81-8,16,4-g37-path3", "128S82-8,16,4-g37-path8", "128S73-8,16,4-g37-path1", "128S74-8,16,4-g37-path4", "128S79-4,16,4-g29-path9", "128S2-8,8,4-g33-path16", "128S80-4,16,4-g29-path23", "128S3-8,8,4-g33-path64", "128S71-4,16,4-g29-path1", "128S77-8,8,4-g33-path5", "128S72-4,16,4-g29-path7", "128S78-8,8,4-g33-path12", "128S26-4,8,4-g25-path31", "128S75-4,8,4-g25-path5", "128S76-4,8,4-g25-path15" ];
