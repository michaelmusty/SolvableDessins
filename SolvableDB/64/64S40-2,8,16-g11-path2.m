...
I<[x]> := ideal< P | [
x[2]*x[3]^4 + 1/2*(-nu^7 + nu^5 - nu^3 + nu)*x[2]^2*x[3]^2 + 1/4*(nu^6 - nu^2)*x[2]^3 + 1/2*(nu^7 - nu^5 + nu^3 - nu)*x[3]^2 + 1/4*(-nu^6 + nu^2)*x[2],
x[1]^2*x[2]^2 + (2*nu^6 - 2*nu^2)*x[1]*x[2]*x[3]^2 + (nu^5 - nu^3)*x[1]*x[2]^2 - x[1]^2 + x[2]^2 + (-nu^5 + nu^3)*x[1] - 1,
x[1]^2*x[3]^2 + (-nu^5 + nu^3)*x[1]*x[3]^2 - x[1]*x[2] + x[3]^2
] >;
...
s`SolvableDBName := "64S40-2,8,16-g11-path2";
s`SolvableDBFilename := "64S40-2,8,16-g11-path2.m";
...
\[ 10, 5, 28, 6, 2, 4, 26, 16, 11, 1, 9, 13, 12, 17, 44, 8, 14, 21, 22, 23, 18, 19, 20, 31, 38, 7, 33, 3, 30, 29, 24, 34, 27, 32, 37, 49, 35, 25, 40, 39, 48, 46, 55, 15, 47, 42, 45, 41, 36, 51, 50, 54, 64, 52, 43, 58, 59, 56, 57, 62, 63, 60, 61, 53 ]:
 Order := 2 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T2-2,2,2-g0-path1", "8T4-2,2,4-g0-path1", "16T13-2,2,8-g0-path1", "32S9-2,4,8-g3-path5", "64S40-2,8,16-g11-path2" ];
s`SolvableDBParents := [ Strings() | "128S114-4,8,16-g37-path7", "128S95-4,8,16-g37-path7", "128S64-4,8,16-g37-path7", "128S82-4,8,16-g37-path1", "128S92-2,8,16-g21-path4", "128S63-2,8,16-g21-path4", "128S81-2,8,16-g21-path1" ];
