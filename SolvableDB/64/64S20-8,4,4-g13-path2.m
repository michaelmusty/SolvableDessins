...
I<[x]> := ideal< P | [
x[2]^3*x[4]^2 - nu*x[1]*x[2]^3 + nu*x[2]^2*x[4]^2 + nu^2*x[1]*x[2]^2 + 1/2*(-nu^3 + nu)*x[2]^3 + nu^2*x[2]*x[4]^2 + nu*x[1]*x[2] + 1/2*(-nu^2 - 1)*x[2]^2 + nu^3*x[4]^2 - nu^2*x[1] + 1/2*(-nu^3 - nu)*x[2] + 1/2*(nu^2 - 1),
x[1]^2*x[2]^2 - x[1]^2 + nu^2*x[2]^2 + nu^2,
x[1]^2*x[4]^2 - nu^2*x[2]^2*x[4]^2 - x[1]^2*x[2] + nu^3*x[1]*x[2]^2 + 1/2*(-nu^3 + nu)*x[1]^2 + 2*x[1]*x[2] + 1/2*(-nu^3 - nu)*x[2]^2 - nu*x[1] - x[2],
x[1]*x[2]*x[4]^2 + nu*x[1]*x[4]^2 + x[2]*x[4]^2 + 1/2*(-nu^3 - nu)*x[1]*x[2] + nu*x[4]^2 + 1/2*(nu^2 - 1)*x[1] + 1/2*(nu^3 + nu)*x[2] + 1/2*(-nu^2 + 1),
x[3]^2 - x[1]
] >;
...
\[ 44, 45, 48, 63, 53, 62, 17, 52, 20, 61, 42, 24, 23, 18, 47, 43, 33, 41, 49, 57, 51, 39, 59, 55, 64, 3, 46, 22, 50, 5, 25, 6, 4, 36, 54, 40, 16, 15, 10, 38, 21, 58, 28, 9, 30, 26, 29, 19, 60, 34, 56, 13, 32, 14, 1, 35, 12, 8, 11, 27, 37, 2, 31, 7 ]:
 Order := 8 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-1,2,2-g0-path1", "4T2-2,2,2-g0-path1", "8T4-4,2,2-g0-path1", "16T10-4,4,2-g1-path5", "32S11-8,4,2-g3-path3", "64S20-8,4,4-g13-path2" ];
s`SolvableDBParents := [ Strings() | "128S11-8,8,4-g33-path17", "128S8-8,4,8-g33-path11", "128S10-8,8,8-g41-path52", "128S32-8,8,4-g33-path10", "128S35-8,4,8-g33-path5", "128S31-8,8,8-g41-path16", "128S27-8,8,4-g33-path17", "128S32-8,4,8-g33-path10", "128S34-8,8,8-g41-path16", "128S20-8,8,4-g33-path7", "128S17-8,4,8-g33-path7", "128S23-8,8,8-g41-path22", "128S29-8,4,4-g25-path4", "128S26-8,4,4-g25-path4", "128S16-8,4,4-g25-path6" ];
