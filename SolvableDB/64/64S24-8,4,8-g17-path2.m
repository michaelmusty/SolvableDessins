...
I<[x]> := ideal< P | [
x[4]^8 + 4*x[1]*x[2]^3 - x[2]^4 + x[3]^4 - 4*x[4]^4 - 4*x[1]*x[2] + 4*x[2]^2 - 4*x[3]^2 + 3,
x[2]*x[4]^6 - 2*x[1]*x[2]^2*x[4]^2 + x[2]^3*x[4]^2 + x[3]^3 + 2*x[1]*x[4]^2 - 2*x[2]*x[4]^2 - x[3],
x[2]^2*x[4]^4 - 2*x[1]*x[2]^3 + x[2]^4 + 2*x[1]*x[2] - 2*x[2]^2 + x[3]^2 - 1,
x[1]^2*x[2]*x[4]^2 - x[1]^2*x[3] + x[2]*x[4]^2 + x[3],
x[1]*x[4]^4 + x[2]*x[4]^4 - x[1]*x[2]^2 + x[2]^3,
x[1]^2*x[2]^2 - x[1]^2 + x[2]^2 + 1,
x[1]*x[3]*x[4]^2 + x[4]^4 - x[1]*x[2] + x[2]^2,
x[1]*x[3]^2 + x[3]*x[4]^2 - x[1] + x[2],
x[2]*x[3] - x[4]^2
] >;
...
phi := KX!(1/(2*x[3]^6)*x[4]^6 + (x[3]^8 - 5)/(4*x[3]^4)*x[4]^4 + (2*x[3]^8 - 4*x[3]^4 + 1)/(2*x[3]^10)*x[4]^2 + (x[3]^8 + 4*x[3]^4 + 3)/(4*x[3]^8));
/* assign to object */
...
s`SolvableDBName := "64S24-8,4,8-g17-path2";
s`SolvableDBFilename := "64S24-8,4,8-g17-path2.m";
...
\[ 6, 1, 27, 28, 24, 29, 4, 30, 2, 3, 5, 7, 8, 23, 48, 62, 13, 14, 64, 35, 39, 31, 61, 9, 21, 42, 34, 44, 11, 55, 19, 20, 22, 41, 57, 51, 59, 10, 12, 15, 16, 17, 18, 25, 26, 32, 33, 36, 37, 60, 53, 63, 54, 58, 45, 50, 49, 40, 46, 43, 56, 38, 47, 52 ]:
 Order := 8 >;
s`SolvableDBPathToPP1 := [ Strings() | "PP1", "2T1-2,1,2-g0-path1", "4T2-2,2,2-g0-path2", "8T4-2,4,2-g0-path2", "16T10-2,4,4-g1-path5", "32S2-4,4,4-g5-path3", "64S24-8,4,8-g17-path2" ];
s`SolvableDBParents := [ Strings() | "128S24-8,8,8-g41-path25", "128S15-8,8,8-g41-path43", "128S22-8,8,8-g41-path13", "128S15-8,8,8-g41-path44", "128S41-8,8,8-g41-path4", "128S31-8,8,8-g41-path37", "128S39-8,8,8-g41-path4", "128S31-8,8,8-g41-path38", "128S17-8,4,8-g33-path9", "128S12-8,4,8-g33-path10", "128S17-8,4,8-g33-path10", "128S28-8,4,8-g33-path25", "128S37-8,4,8-g33-path7", "128S27-8,4,8-g33-path19", "128S37-8,4,8-g33-path8" ];
